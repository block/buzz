...
The test is ignored by default and fails before connecting when `BUZZ_TEST_DATABASE_URL` is absent. It passed against a freshly initialized PostgreSQL 17.11 cluster after all embedded migrations, including a new `Db` connection that simulated scheduler restart for approval resume (`1 passed; 0 failed`). A separate process-level smoke then booted the compiled relay twice over the same PostgreSQL 17.11 cluster and a signed Memurai 4.1.2 server (Redis API 7.2.5): both boots reconnected Postgres/Redis, started the workflow cron loop, preserved the same deployment community, and returned `200 {"status":"ready"}`. `BUZZ_GIT_CONFORMANCE_PROBE=false` isolated the unrelated S3/Git gate; no persistent service was installed, and all temporary runtimes were removed.
