...
- 验证：Xiaoxin 现场复现漏检，并确认 Disconnect Buzz 后 Agent harness 退出、shared app-server 和 Codex Desktop backend 均继续存活，离线 Agent 的 DM 入口与输入框仍可用。Tauri `codex_desktop` 定向测试 11/11 通过，新增用例覆盖 LocalAppData Desktop 子进程、旧 Appx backend、shared listener 与无关 CLI backend。
- 版本/提交：基于 `310de4e2`，分支 `codex/mobile-lan-relay-fast-path`；待提交。
