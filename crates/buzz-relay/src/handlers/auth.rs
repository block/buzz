...
pub async fn handle_auth(event: nostr::Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
        ...
        .await
    {
        Ok(mut auth_ctx) => {
            ...
            // Register the proven key together with its admitting NIP-FI issuer
            // (the same string the deny check below uses) BEFORE the deny-set
            // check, so an issuer-scoped close scan can match this session.
...
mod tests {
    ...
    /// Like `auth_test_state` but connects to a real Postgres.
    ///
    ...
    ///
    /// The DB is the PG lane wrapper's per-test database; a missing or
    /// unreachable DB panics — these witnesses never skip.
    async fn auth_test_state_real_db() -> std::sync::Arc<crate::state::AppState> {
        use std::sync::Arc;
    ...
    //
    // Runs in PG lane on wrapper DB: the ban-check path precedes the hook. The DB call returns
    // "not banned" for an unknown community/pubkey — a real result, not mocked.
    async fn w1_auth_barrier_expiry_mid_flight_blocks_auth_commit_body() {
        use buzz_auth::VerifiedAssertion;
        use chrono::{Duration, Utc};
        use std::collections::HashMap;
        use std::sync::Arc;
        ...
        let state = auth_test_state_real_db().await;
        let relay_url = "ws://test.local";
        let auth_event = EventBuilder::new(Kind::Authentication, "")
            .tag(Tag::parse(["relay", relay_url]).unwrap())
            .tag(Tag::parse(["challenge", &challenge]).unwrap())
    ...
    async fn w_deny_straddle_entry_inserted_between_registration_and_check_is_caught_body() {
        ...
        let mut state =
            Arc::try_unwrap(auth_test_state_real_db().await).unwrap_or_else(|arc| (*arc).clone());
    ...
    mod postgres_tests {
        ...
        async fn w1_auth_barrier_expiry_mid_flight_blocks_auth_commit() {
            super::w1_auth_barrier_expiry_mid_flight_blocks_auth_commit_body().await;
        }

        #[tokio::test]
        #[ignore = "requires Postgres"]
        async fn w_deny_straddle_entry_inserted_between_registration_and_check_is_caught() {
            super::w_deny_straddle_entry_inserted_between_registration_and_check_is_caught_body()
                .await;
        }
        ...
        async fn w_deny_other_issuer_straddle_leaves_session_admitted() {
            super::w_deny_other_issuer_straddle_leaves_session_admitted_body().await;
        }
    }
}
