...
use buzz_core::kind::{
    ...
    KIND_PLANNING_PLAYBOOK, KIND_PLANNING_PROJECT, KIND_PLANNING_TASK, KIND_PLANNING_TASK_ARTIFACT,
    KIND_PLANNING_TASK_DETAILS, KIND_PLANNING_TASK_EXECUTION, KIND_PRESENCE_UPDATE,
...
fn required_scope_for_kind(kind: u32, event: &Event) -> Result<Scope, &'static str> {
    match kind {
        ...
        | KIND_PLANNING_TASK_EXECUTION | KIND_PLANNING_TASK_ARTIFACT
        | KIND_PERSONA | KIND_TEAM | KIND_MANAGED_AGENT
...
mod tests {
    ...
    #[test]
    fn planning_events_are_owner_global_user_writes() {
        for kind in [
            KIND_PLANNING_PROJECT,
            KIND_PLANNING_TASK,
            KIND_MISSION_CONSTRAINT,
