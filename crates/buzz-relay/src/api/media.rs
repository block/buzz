...
pub async fn upload_blob(
    ...
    while sniff.len() < SNIFF_BYTES {
        match source.next().await {
            ...
            // Classify by error type: a body tripping one of the media
            // router's reception deadlines is the client's fault (408), a body-limit
            ...
            Some(Err(error)) => {
                return Err(match buzz_media::classify_body_error(&error) {
                    buzz_media::BodyErrorKind::Timeout => MediaError::RequestBodyTimeout,
