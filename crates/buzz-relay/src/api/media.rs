...
pub async fn upload_blob(
    ...
    let mut descriptor = if should_stream_as_video(&sniff) {
        ...
        // `to_bytes` errors on either the inner (buffered) limit or a stream
        // error from the layered body. Classify instead of assuming 413: an
        // idle-deadline trip must answer 408, not read as too-large or as a
        // storage failure.
        let bytes = axum::body::to_bytes(axum::body::Body::from_stream(replay), max as usize)
            ...
            .map_err(|error| match buzz_media::classify_body_error(&error) {
                buzz_media::BodyErrorKind::IdleTimeout => MediaError::RequestBodyTimeout,
                buzz_media::BodyErrorKind::LengthLimit => MediaError::FileTooLarge { size: 0, max },
                buzz_media::BodyErrorKind::Other => MediaError::Io(error.to_string()),
            })?;
