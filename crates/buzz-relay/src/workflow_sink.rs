...
impl ActionSink for RelayActionSink {
    ...
    ) -> Pin<Box<dyn Future<Output = Result<AddReactionOutcome, ActionSinkError>> + Send + '_>>
    {
        ...
        Box::pin(async move {
            ...
            let target_author = crate::handlers::ingest::effective_message_author(
                &target_event.event,
                &state.relay_keypair.public_key(),
            );
            let target_author_hex = nostr::PublicKey::from_slice(&target_author)
                .map_err(|e| {
                    ActionSinkError::InvalidInput(format!(
                        "reaction target has an invalid effective author: {e}"
                    ))
                })?
                .to_hex();
            ...
            // deletion, and reaction summaries. The target-author p tag keeps
...
mod postgres_tests {
    ...
    async fn workflow_add_reaction_persists_attributed_kind_7_and_dedupes() {
        ...
        assert!(stored
            ...
            .tags
            .iter()
            .any(|tag| { tag.as_slice() == ["p".to_string(), owner_hex.clone()] }));
        assert!(stored
            .event
            .tags
            .iter()
