...
impl DevMcp {
    ...
    #[tool(
        name = "relay_reply",
        description = "Send one durable reply to a Buzz message through the harness-owned broker. Use this for all Buzz replies: it accepts only channel_id, reply_to, content, and an idempotency key, and never exposes a signing key. If delivery outcome is unknown, retry the exact same request with the returned idempotency_key; do not create a fresh reply."
    )]
    async fn relay_reply(
        &self,
        Parameters(params): Parameters<reply::ReplyParams>,
    ) -> Result<String, ErrorData> {
        reply::send(params).await
    }
