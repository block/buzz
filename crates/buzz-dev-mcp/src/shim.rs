...
///
/// Shell children receive `path_env` and `git_env`, never raw signing
/// credentials. `NOSTR_PRIVATE_KEY` is removed from the process env after the
/// keyfile is written — git helpers read from the keyfile only.
