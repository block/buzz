...
use crate::{kind::KIND_CLIENT_BINDING_STATUS, verify_event, CommunityId};
...
/// Maximum encoded length of the optional privacy-approved display label.
pub const MAX_CLIENT_BINDING_STATUS_LABEL_BYTES: usize = 80;
...
impl fmt::Debug for ClientBindingStatusV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field(
                "display_label",
                &self.display_label.as_ref().map(|_| "[redacted]"),
            )
            .finish()
    }
}
...
impl ClientBindingStatusV1 {
    ...
    /// Optional privacy-approved label for current presentation.
...
pub fn validate_client_binding_status_event(
    ...
    let display_label = wire.display_label;
...
fn validate_payload_fields(
    ...
    match disposition {
        ...
        ClientBindingStatusDisposition::Withdrawn => {
            if binding_version.is_some() || policy_version.is_some() || display_label.is_some() {
    ...
    validate_display_label(disposition, display_label)
}

fn validate_display_label(
    disposition: ClientBindingStatusDisposition,
    display_label: Option<&str>,
) -> Result<(), ClientBindingStatusError> {
    let Some(label) = display_label else {
        return Ok(());
    };
    if disposition != ClientBindingStatusDisposition::DisplayCurrent
        || label.is_empty()
        || label.len() > MAX_CLIENT_BINDING_STATUS_LABEL_BYTES
        || label.trim() != label
        || label.chars().any(char::is_control)
    {
        return Err(ClientBindingStatusError::InvalidDisplayLabel);
    }
    Ok(())
}
...
/// author tuple. A lower revision or a different event at the same revision is
/// rejected. Expiry and disconnect clear presentation while retaining the
/// high-water mark, so a previously seen envelope cannot restore a badge.
...
impl ClientBindingStatusTracker {
    ...
    pub fn accept(
        ...
        if let Some(high_water) = self.high_water {
            ...
            if status.status_revision == high_water.revision {
                if status.event_id != high_water.event_id {
...
pub enum ClientBindingStatusError {
    ...
    Expired,
    /// The optional display label was unsafe, out of bounds, or attached to a
    /// non-current disposition.
    #[error("client binding status display label is invalid")]
    InvalidDisplayLabel,
}
...
mod tests {
    ...
    fn validates_relay_authenticated_current_status() {
        ...
        assert_eq!(status.display_label(), Some("Synthetic Example"));
    ...
    fn withdrawal_removes_current_presentation_without_lifecycle_data() {
        ...
        assert!(status.display_label().is_none());
    }
    ...
    #[test]
    fn label_is_privacy_bounded_and_current_only() {
        let author = Keys::generate();
        for label in [
            "",
            " synthetic.example",
            "synthetic.example\n",
            &"x".repeat(MAX_CLIENT_BINDING_STATUS_LABEL_BYTES + 1),
        ] {
            assert!(matches!(
                ...
                Err(ClientBindingStatusError::InvalidDisplayLabel)
            ));
        }
        let withdrawal = ClientBindingStatusInputV1::withdrawn(
            domain(),
            author.public_key(),
            11,
            ISSUED_AT,
        ...
        .expect("withdrawal needs no label");
        assert!(!format!("{withdrawal:?}").contains("Synthetic Example"));
    }
