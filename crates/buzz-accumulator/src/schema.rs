...
impl ArtifactSchema {
    ...
    pub fn contract_prompt(&self) -> String {
        let mut s = format!(
            "OUTPUT CONTRACT (the section structure is mechanically enforced; output with \
             different H1 sections is refused and nothing persists):\nWhatever the task above \
             asks for, deliver it as markdown with exactly these H1 sections, in this order: \
             {}.\n",
            self.sections.join(", ")
        );
        ...
        s.push_str(
            "Cite the source events you actually use as [event:<id>], one id per bracket, \
             copied in full from the SOURCE EVENT IDS list — citations render as clickable \
             links to the source, so a wrong or invented id shows readers a dead link. Do \
             not invent facts. Output only the document markdown.",
        );
        s
    }
}
