...
pub struct BlobDescriptor {
    ...
    /// Original client-side filename. The content-addressed relay does not
    /// persist it, but message imeta uses it for attachment labels.
...
fn sanitize_upload_filename(file_path: &str) -> String {
    let base = file_path
        .rsplit(['/', '\\'])
        .next()
        .unwrap_or(file_path)
...
mod retry_policy_tests {
    ...
    #[tokio::test]
    async fn upload_accepts_markdown_and_preserves_its_filename() {
        use tokio::io::{AsyncReadExt, AsyncWriteExt};

        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("release-notes.md");
        std::fs::write(&path, b"# Release notes\n\nOne-click message sending.\n").unwrap();
