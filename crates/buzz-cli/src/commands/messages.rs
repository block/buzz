...
pub async fn cmd_edit_message(
    ...
    validate_hex64(event_id)?;
    // Allow '-' to read the replacement from stdin, exactly as `send` does.
    // Without this the two write paths disagree on what '-' means, so
    // `buzz messages edit --content -` stores a literal "-" and destroys the
