...
use crate::client::{extract_d_tag, extract_p_tags, normalize_write_response, BuzzClient};
use crate::error::CliError;
use crate::validate::{parse_uuid, sdk_err, validate_hex64};

/// List DM conversations.
///
/// DMs are NIP-29 channels with `channel_type = "dm"` — the relay never emits a
/// kind:41001 confirmation event (that kind is reserved but unused), so this
/// follows the same discovery path as `channels list --member`: find channel
/// ids from our kind:39002 membership events, then fetch kind:39000 metadata
/// for those ids and keep only the ones tagged `["t", "dm"]`.
pub async fn cmd_list_dms(client: &BuzzClient, limit: Option<u32>) -> Result<(), CliError> {
    ...
    let is_dm = |e: &serde_json::Value| -> bool {
        e.get("tags")
    ...
    let mut dms: Vec<serde_json::Value> = metadata_events
        ...
        .map(|e| {
            ...
            let participants: Vec<String> = extract_p_tags(e)
                .into_iter()
                .filter_map(|p| p.get("pubkey")?.as_str().map(|s| s.to_string()))
                .collect();
            serde_json::json!({
                "dm_id": dm_id,
