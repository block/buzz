...
pub enum NotesCmd {
    ...
    ///
    /// v1 implementation emits an `e`-tag deletion of the current event id
    /// (works today) plus an `a`-coordinate tag (forward-compatible; becomes
    /// authoritative once the relay's NIP-09 a-tag path handles 30023).
    Rm {
