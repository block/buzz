...
//! - `notes rm --name <s>` — NIP-09 deletion targeting the current event id
//!   (an `a`-tag is also included for forward-compatibility).
//!
//! ## Implementation state
//! `set`/`get`/`ls` are implemented. `set` is the write path: stdin handling,
...
//! bounded listing). `rm` remains a stub pending the relay-side a-tag deletion
//! fix (#714); its helpers stay live for that follow-up.
#![allow(dead_code)]
...
use nostr::{Event, EventBuilder, Filter, Kind, PublicKey, Tag, Timestamp, ToBech32};
...
///
/// Stays protocol-pure (returns a raw `Event`) so `rm` can grab `.id` without
/// re-deriving anything; `set` calls `NoteSnapshot::from_event` on the result
/// for carry-forward. (Quinn's option (b) — single tag-parser, isolated.)
...
#[allow(dead_code)] // Used once `get`/`set` land; kept for the public helper API.
...
#[allow(dead_code)] // Convenience for the verbs; suppress until they land.
pub fn filter_for_coord(coord: &nostr::nips::nip01::Coordinate) -> Filter {
    Filter::from(coord)
}
...
pub async fn cmd_rm(_client: &SproutClient, _slug: &str) -> Result<(), CliError> {
    Err(CliError::Other("notes rm: not yet implemented".into()))
}
