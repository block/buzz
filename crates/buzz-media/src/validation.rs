...
///
/// The rules themselves live in `buzz-image`, alongside the sanitizer that has
/// to satisfy them. Keeping the two together is deliberate: they are only ever
/// correct relative to each other, and when they lived in separate crates they
/// drifted — the desktop app stripped metadata, the CLI did not, and agents
/// could not upload a screenshot or a chart at all.
fn validate_image_metadata_free(bytes: &[u8], mime: &str) -> Result<(), MediaError> {
    buzz_image::validate_image_metadata_free(bytes, mime).map_err(MediaError::from)
}

// Per-format entry points, kept so the format-specific tests below (and any
// caller wanting to be explicit) read the same as they always did. These hold
// no logic of their own.
#[cfg(test)]
fn validate_jpeg_metadata_free(bytes: &[u8]) -> Result<(), MediaError> {
    validate_image_metadata_free(bytes, "image/jpeg")
}
...
fn validate_webp_metadata_free(bytes: &[u8]) -> Result<(), MediaError> {
    validate_image_metadata_free(bytes, "image/webp")
}

#[cfg(test)]
fn validate_gif_metadata_free(bytes: &[u8]) -> Result<(), MediaError> {
    validate_image_metadata_free(bytes, "image/gif")
}
