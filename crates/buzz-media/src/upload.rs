...
pub async fn process_video_upload(
    ...
    let (sha256_hex, file_size, first_bytes) = {
        ...
        //   - reception deadline trip (typed tower-http TimeoutError or
        //     BodyDeadlineError in the source chain) -> TimedOut ->
        ...
        let mapped = futures_util::StreamExt::map(body_stream, |r| {
            r.map_err(|e| match crate::error::classify_body_error(&e) {
                crate::error::BodyErrorKind::Timeout => {
        ...
        loop {
            ...
            let n = match reader.read(&mut buf).await {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::TimedOut => {
                    // A reception deadline fired — the client did not deliver
                    // the body in time. 408, never 500: not a storage failure.
...
mod tests {
    ...
    async fn classify_body_error_detects_real_tower_http_timeout_by_type() {
        ...
        assert_eq!(
            ...
            BodyErrorKind::Timeout,
            "typed TimeoutError must classify as Timeout through the axum::Error wrapping"
        );

        // The reception ceiling's own error classifies identically, so both
        // deadlines answer 408 at every consumption path.
        let ceiling = axum::Error::new(crate::error::BodyDeadlineError);
        assert_eq!(classify_body_error(&ceiling), BodyErrorKind::Timeout);
    ...
    async fn video_stream_idle_timeout_maps_to_request_body_timeout_not_500() {
        ...
        // never happened.
        //
        // Scope: this test proves the conversion, NOT the video routing.
        // Mutating the chunk content does not change the outcome. The read loop
