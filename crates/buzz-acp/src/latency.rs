...
impl LatencyCollector {
    ...
    pub(crate) fn ingest(&mut self, event: &ObserverEvent) -> Option<CompletedLatency> {
        ...
        match event.kind.as_str() {
            ...
            "reply_observed" => {
                // Signed thread tags identify which triggering event this reply
                // answers. Prefer that evidence over the currently active turn:
                // relay fanout for turn A can arrive after turn B has started.
                let tagged_event_ids: Vec<String> = ["parentEventId", "rootEventId"]
                    .iter()
                    .filter_map(|key| string_field(&event.payload, key))
                    .collect();
                let tagged_turn_id = tagged_event_ids
                    .iter()
                    .find_map(|event_id| self.event_to_turn.get(event_id).cloned());
                candidate_turn_id = if tagged_event_ids.is_empty() {
                    self.unambiguous_channel_turn(
                        event.channel_id.as_deref(),
                        candidate_turn_id.as_deref(),
                    )
                } else {
                    // Signed correlation evidence is authoritative. If none of
                    // the tags match a tracked trigger, fail closed instead of
                    // assigning an unrelated threaded reply to the active turn.
                    tagged_turn_id
                };
    ...
    fn unambiguous_channel_turn(
        &mut self,
        channel_id: Option<&str>,
        contextual_turn_id: Option<&str>,
    ) -> Option<String> {
        if let Some(turn_id) = contextual_turn_id {
            if self
                .turns
                .get(turn_id)
                .is_some_and(|turn| turn.reply_observed_ms.is_some())
            {
                return Some(turn_id.to_string());
            }
        }

        // Flat human-facing replies have no signed thread markers. If a
        // completed no-reply turn and a newer active turn overlap, the relay
        // event cannot identify either one safely. Drop that sample instead of
        // manufacturing plausible zeroes or polluting the newer turn.
        let unanswered: Vec<String> = self
            .turns
            .iter()
            .filter(|(_, turn)| {
                turn.channel_id.as_deref() == channel_id && turn.reply_observed_ms.is_none()
            })
            .map(|(turn_id, _)| turn_id.clone())
            .collect();
        let only_turn_id = unanswered.first()?;
        if unanswered.len() > 1 {
            // This fanout cannot be attributed safely. Retire terminal traces
            // that already missed their reply boundary so they do not keep
            // every later flat reply ambiguous until the TTL expires.
            let terminal: Vec<String> = unanswered
                .into_iter()
                .filter(|turn_id| {
                    self.turns
                        .get(turn_id)
                        .is_some_and(|turn| turn.completed_ms.is_some())
                })
                .collect();
            for turn_id in terminal {
                self.remove_trace(&turn_id);
            }
            return None;
        }
        match contextual_turn_id {
            Some(contextual) if contextual == only_turn_id => Some(contextual.to_string()),
            None => Some(only_turn_id.clone()),
            _ => None,
        }
    }

    fn remove_trace(&mut self, turn_id: &str) {
        let Some(turn) = self.turns.remove(turn_id) else {
            return;
        };
...
mod tests {
    ...
    #[test]
    fn unmatched_signed_reply_does_not_fall_back_to_the_active_turn() {
        let mut collector = LatencyCollector::default();
        collector.ingest(&event(
            "event_received",
            10,
            None,
            json!({"eventId": "trigger-1", "receiptToObserverMs": 0}),
        ));
        collector.ingest(&event(
            "event_queued",
            11,
        ...
        assert!(
            collector
                .ingest(&event(
                    ...
                    Some("turn-1"),
                    json!({
                        "eventId": "unrelated-reply",
                        "parentEventId": "untracked-thread",
                    }),
                ))
                .is_none(),
            "an unmatched signed reply must not emit a sample"
        );
        assert_eq!(
            collector
                .turns
                .get("turn-1")
                .and_then(|turn| turn.reply_observed_ms),
            None,
            "an unmatched signed reply must not be assigned to the active turn",
        );
    }

    #[test]
    fn delayed_reply_tags_outrank_a_newer_active_turn() {
        let mut collector = LatencyCollector::default();
        for (event_id, turn_id, received, started) in [
            ("trigger-a", "turn-a", 10, 15),
            ("trigger-b", "turn-b", 30, 35),
        ] {
            collector.ingest(&event(
                "event_received",
                received,
        ...
        collector.ingest(&event("turn_completed", 40, Some("turn-a"), json!({})));
        ...
        let completed = collector
            .ingest(&event(
                ...
                json!({
                    "eventId": "reply-a",
                    "parentEventId": "trigger-a",
                }),
            ))
            .expect("signed parent tag should close turn A");

        assert_eq!(completed.payload["traceId"], "trigger-a");
        assert_eq!(
            collector
                .turns
                .get("turn-b")
                .and_then(|turn| turn.reply_observed_ms),
            None,
            "delayed turn A fanout must not pollute active turn B",
        );
    }

    #[test]
    fn ambiguous_flat_reply_drops_the_terminal_trace_and_recovers() {
        let mut collector = LatencyCollector::default();
        for (event_id, turn_id, received, started) in [
            ("trigger-a", "turn-a", 10, 15),
            ("trigger-b", "turn-b", 30, 35),
        ...
        assert!(
            collector
                ...
                .is_none(),
            "an ambiguous flat reply must not emit a sample"
        );
        assert!(
            !collector.turns.contains_key("turn-a"),
            "the terminal ambiguous trace must be retired"
        );
        assert_eq!(
            collector
                .turns
                ...
                .and_then(|turn| turn.reply_observed_ms),
            None,
            "the ambiguous reply must not be assigned to active turn B",
        );
        ...
        assert!(
            collector
                ...
                .is_none(),
            "turn B still needs its completion boundary"
        );
        let completed = collector
            .ingest(&event("turn_completed", 55, Some("turn-b"), json!({})))
            .expect("turn B should recover after the ambiguous terminal trace is retired");
        assert_eq!(completed.payload["traceId"], "trigger-b");
    }
