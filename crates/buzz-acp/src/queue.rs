...
///    Legacy agents only, and only on the session's first message (see
///    `standing_context_sent`)
/// 1. `[Task Handoff]` — the first turn after loading an external task
/// 2. `[Buzz Delivery]` — reminder for shared-runtime task-bound delivery
