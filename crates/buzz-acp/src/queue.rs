...
/// Unit of turn concurrency: one thread within a channel, or the channel's
/// top-level lane when the event has no thread root.
///
...
pub struct ConversationKey {
    pub channel_id: Uuid,
    /// Thread root event id (hex) for threaded replies; `None` for the
    /// channel's top-level lane.
