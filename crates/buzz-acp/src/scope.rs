...
impl ConversationScope {
    ...
    ///   [`buzz_core::thread::parse_nip10_thread`]) selects the thread scope —
    ///   this covers stream thread replies (kind
    ...
    pub fn for_event(channel_id: Uuid, event: &nostr::Event, thread_scoped: bool) -> Self {
        ...
        let root = buzz_core::thread::parse_nip10_thread(event).map(|thread| thread.root_event_id);
        match root {
...
mod tests {
    ...
    #[test]
    fn root_only_marker_is_unthreaded() {
        let ch = Uuid::new_v4();
    ...
    fn same_thread_same_scope_distinct_threads_distinct_scopes() {
        let ch = Uuid::new_v4();
        let root_a = vec!["e".into(), "a".repeat(64), "".into(), "reply".into()];
        let root_b = vec!["e".into(), "b".repeat(64), "".into(), "reply".into()];
    ...
    fn forum_comment_resolves_to_the_posts_scope() {
        ...
        // A direct comment uses the SDK's canonical reply-only encoding.
        let comment = buzz_sdk::build_forum_comment(
            ch,
            "comment",
            &buzz_sdk::ThreadRef {
                root_event_id: post.id,
                parent_event_id: post.id,
            },
            &[],
            &[],
        )
        .expect("build forum comment")
        .sign_with_keys(&Keys::generate())
        .expect("sign forum comment");
    ...
    #[test]
    fn sdk_direct_and_nested_replies_preserve_canonical_scope() {
        let ch = Uuid::new_v4();
        let keys = Keys::generate();
        let root = buzz_sdk::build_message(ch, "root", None, &[], false, &[])
            .expect("build root")
            .sign_with_keys(&keys)
            .expect("sign root");
        ...
        .expect("build nested reply")
        .sign_with_keys(&keys)
        .expect("sign nested reply");

        let expected = ConversationScope::thread(ch, root.id);
    ...
    #[test]
    fn malformed_later_duplicate_does_not_erase_valid_scope_marker() {
        let ch = Uuid::new_v4();
        let parent = "b".repeat(64);
        let event = make_event(vec![
            vec!["e".into(), root_hex(), "".into(), "root".into()],
        ...
        assert_eq!(
            ConversationScope::for_event(ch, &event, true),
            ConversationScope::thread(ch, EventId::from_hex(&root_hex()).expect("valid root"))
        );
    }
}
