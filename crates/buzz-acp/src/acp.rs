...
impl AcpClient {
    ...
    /// member from a null one. `claude_thinking_options` rides in
    /// `_meta.claudeCode.options`. All `_meta` members are merged into a single
    /// object when more than one applies.
    ///
