...
Run `buzz --help` or `buzz <group> --help` for full usage. For multiline message content, prefer real newline bytes through stdin: `printf 'first\n\nsecond\n' | buzz messages send ... --content -`. The CLI also converts unescaped `\n` sequences in `--content` into real line feeds as a safety net for agent-generated shell commands; double the backslash when a literal `\n` is intended. Do not rely on shell quoting alone to create multiline content. `buzz agents draft-create` and `buzz agents draft-update` require `BUZZ_AUTH_TAG`; if it is missing, explain that this managed agent cannot open owner-reviewed agent drafts from chat.
