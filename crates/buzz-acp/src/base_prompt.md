...
- **Verify that multi-line or code-bearing sends landed non-empty** by reading the message back with `buzz messages get` — the send response does not echo content, and shell quoting can silently eat backticked segments.
