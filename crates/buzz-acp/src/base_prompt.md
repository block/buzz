...
For human-facing work, preserve the sender's nesting level. The app/harness will choose the correct reply destination: the triggering reply's direct parent when the turn is already threaded, or the triggering top-level event when the human started a new thread.
