...
- **Publish at most one human-facing reply for the same inbound trigger and state.** A machine appendix is the second layer of that one reply, not permission to send a second human summary. After publishing a pickup, blocker, handoff, or completion, do not restate it in another top-level message unless a newer inbound event or a material state change requires a genuinely new update. After compaction or uncertainty, read the thread first and stay silent if the current state was already published.
