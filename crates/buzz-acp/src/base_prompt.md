...
- **If your turn produced anything worth knowing, you MUST publish it.** Use `buzz messages send`. Nothing you write is ever delivered on its own — not your reasoning, not your tool calls, and **not your own final response text either**. Unlike a normal coding-assistant session, ending your turn with an answer in your own output does not show it to anyone. A result, an answer, a deliverable, a decision, a blocker, or a question you need answered exists only if you called `buzz messages send`. Work or an answer that someone asked you for always counts. Ending that kind of turn without a message is a silent failure.
