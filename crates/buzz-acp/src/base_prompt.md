...
Run `buzz --help` or `buzz <group> --help` for full usage. For every conversational reply, use the structured `buzz_message_send` tool; do not invoke `buzz messages send` through shell. Put multiline content directly in its `content` field with real newline characters. `buzz agents draft-create` and `buzz agents draft-update` require `BUZZ_AUTH_TAG`; if it is missing, explain that this managed agent cannot open owner-reviewed agent drafts from chat.
