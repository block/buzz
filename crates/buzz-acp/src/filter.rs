...
///    access. Skipped when `mention_exempt` is `true` — the caller sets this for
///    DM channels, where a message addresses the agent implicitly.
...
mod tests {
    ...
    #[tokio::test]
    async fn test_match_event_mention_exempt_skips_mention_gate() {
        let agent_pubkey = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";

        // No p tag — would be dropped by the mention gate in a stream channel.
        let event_no_mention = make_event(9, "hello");
        let channel_id = any_channel();
        ...
        // mention_exempt=true (DM channel) — matches despite the missing p tag.
