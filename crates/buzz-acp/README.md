...
| `--dm-autopublish` | `BUZZ_ACP_DM_AUTOPUBLISH` | `true` | In DM channels, when a turn ends without the agent having published a message, post the agent's trailing reply text as a top-level message so the human never gets silence. Skipped when the agent already posted in that turn; channels are unaffected. |
