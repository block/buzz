...
A failure that is *not* an answer — a timeout, a broken pipe, an agent that exits mid-load — is treated differently: the mapping is kept and the turn is retried, because the session may well still be there and the load may even have landed on the agent's side. Opening a fresh session then would leave two live sessions for one channel. After three such attempts the harness gives up on the mapping and starts fresh, so a channel cannot stall indefinitely.

The remembered entry also records the **model** and **permission mode** the session was created under, and is refused when either has since changed. Neither can be applied by `session/load` — both are resolved from the `session/new` response — so resuming across such a change would silently keep running the old configuration. Everything else the harness sends (system prompt, team instructions, MCP servers, agent memory) is delivered per turn and does not invalidate a session.
