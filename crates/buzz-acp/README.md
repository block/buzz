...
A resume that cannot happen never costs the turn: if the file is missing, the entry is stale, the agent does not advertise `loadSession`, or the agent **answers** `session/load` with an error, the harness forgets the mapping and falls back to `session/new` for that channel.

A failure that is *not* an answer — a timeout, a broken pipe, an agent that exits mid-load — is treated differently: the mapping is kept and the turn is retried, because the session may well still be there and the load may even have landed on the agent's side. Opening a fresh session then would leave two live sessions for one channel. Repeated transport failures remain indeterminate and never discard the mapping; an operator must use an intentional fresh-session transition such as `!rotate` when abandoning it is the correct recovery.

The remembered entry also records the **model** and **permission mode actually applied** when the session was created, and is refused when either has since changed. An unsupported requested model or permission mode is not recorded as effective. Neither setting can be applied by `session/load` — both are resolved from the `session/new` response — so resuming across such a change would silently keep running the old configuration. Everything else the harness sends (system prompt, team instructions, MCP servers, agent memory) is delivered per turn and does not invalidate a session.

Each file also records the relay and agent pubkey it belongs to and is ignored when either differs, so one agent identity serving several communities cannot resume one community's session in another. Every intentional fresh-session transition — `!rotate`, a model switch, a token/turn-limit rotation, leaving the channel — clears the remembered entry along with the in-memory one. An agent *crash* deliberately does not: the harness still wants the conversation back when the agent respawns.

> **Operators running in containers:** the state directory and its file are created owner-only (`0700` / `0600`). The default state directory lives under the working directory, which is ephemeral in most container and pod filesystems — it silently vanishes on every deploy, so every restart starts every channel from scratch. Set `BUZZ_ACP_STATE_DIR` to a path on mounted persistent storage (a volume, PVC, or bind mount, e.g. `/var/lib/buzz-acp`) when the harness does not live on a laptop. Gateways that supervise hosted harnesses can additionally key sessions by `(agent pubkey, channelId)` in their own store — the harness sends the channel on `session/new` as `_meta.channelId` — to resume across a node move even when the file did not survive.
