...
/// - Collapses loopback variants (`localhost`, `127.0.0.0/8`, `[::1]`) to
///   `127.0.0.1` so the auth layer agrees with the community-binding layer.
///
/// **Loopback-only aliasing.** Only loopback address spellings are collapsed
/// — `normalize_loopback_host` does not strip trailing dots, default ports,
/// or merge any routable/public host into another. This prevents widening
/// NIP-98 `u`-tag matching beyond the loopback set. Tests reconstruct
/// `expected_url` from their own bound host, the same shape production does.
fn normalize_url(raw: &str) -> String {
    let mut parsed = match Url::parse(raw) {
        Ok(u) => u,
        Err(_) => return raw.to_lowercase(),
    };
    // Collapse loopback variants only (localhost, 127.0.0.0/8, [::1]) → 127.0.0.1.
    // We deliberately call `normalize_loopback_host` directly instead of
    // `normalize_host` to avoid widening NIP-98 `u`-tag matching beyond
    // loopback — `normalize_host` also strips trailing dots and default ports,
    // which would be a security-relevant equivalence widening. Loopback-only
    // collapsing is safe and necessary so the auth layer agrees with the
    // community-binding layer on a single canonical loopback identity.
    if let Some(host) = parsed.host_str() {
        let port = parsed.port().map(|p| format!(":{p}")).unwrap_or_default();
        let authority = format!("{host}{port}");
        let normalized = buzz_core::tenant::normalize_loopback_host(&authority);
