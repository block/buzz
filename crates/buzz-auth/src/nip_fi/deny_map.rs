...
impl NipFiDenyMap {
    ...
    /// Hex encoding of `pubkey`, for downstream logging and wire use.
    pub fn pubkey_hex(pubkey: &PublicKey) -> String {
        pubkey.to_hex()
    }

    /// Poisons `iss`'s shard mutex so tests outside this crate can drive the
    /// fail-closed `ShardPoisoned` paths.
    #[cfg(any(test, feature = "test-utils"))]
    pub fn poison_shard_for_test(&self, iss: &str) {
        let shards = Arc::clone(&self.shards);
        let iss = iss.to_owned();
        let _ = std::thread::spawn(move || {
            let shard = shards.get(&iss).expect("shard must exist");
            let _guard = shard.lock();
            panic!("intentional shard poison (test)");
        })
        .join();
    }
}
