...
/// upper edge so concurrent inserts cannot make one scan grow without bound.
...
mod postgres_tests {
    ...
    async fn scheduled_workflow_pages_include_rows_after_global_limit() {
        ...
        assert_eq!(
            second.len(),
            1,
            "row 1,001 must be scanned while the post-snapshot insert stays out"
        );
        assert_eq!(second[0].name, "scheduled-1001");
    }
