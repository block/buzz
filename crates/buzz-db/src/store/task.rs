...
impl crate::Db {
    /// Create a task and its opening event.
    pub async fn create_task(&self, community: CommunityId, new_task: NewTask) -> Result<TaskRecord> {
        create_task(&self.pool, community, new_task).await
    }

    /// Fetch a single task by id.
    pub async fn get_task(&self, community: CommunityId, id: Uuid) -> Result<TaskRecord> {
        get_task(&self.pool, community, id).await
    }
    ...
    pub async fn list_tasks(
        ...
        list_tasks(&self.pool, community, filter).await
    }

    /// List the event log for a task.
    pub async fn list_task_events(
        &self,
        community: CommunityId,
        task_id: Uuid,
    ) -> Result<Vec<TaskEventRecord>> {
        list_task_events(&self.pool, community, task_id).await
    }

    /// Apply a patch to a task, appending the derived events.
    pub async fn update_task(
        &self,
        community: CommunityId,
        id: Uuid,
        ...
        update_task(&self.pool, community, id, patch, actor_pubkey).await
    }

    /// Append a single task event.
    pub async fn append_task_event(
        &self,
        community: CommunityId,
        task_id: Uuid,
        ...
        append_task_event(&self.pool, community, task_id, actor_pubkey, action, body).await
    }
}
