...
pub struct WorkflowLifecycleDeleteResult {
    ...
    /// Channels whose workflow caches must be invalidated.
    pub affected_channel_ids: Vec<Uuid>,
...
/// no-op. The result includes every affected channel needed to invalidate the
/// per-channel trigger cache, including orphan definition-only repair.
/// A deletion only affects definition versions at or before its own
/// `created_at`; if a newer live definition exists, the workflow row is kept.
pub async fn delete_workflow_and_definition_for_owner(
    ...
    let row = sqlx::query(
        "DELETE FROM workflows WHERE community_id = $1 AND id = $2 AND owner_pubkey = $3 \
           AND NOT EXISTS ( \
               ...
               WHERE community_id = $1 AND kind = $4 AND pubkey = $3 AND d_tag = $5 \
    ...
    if let Some(channel_id) = workflow_channel_id {
...
mod tests {
    ...
    async fn workflow_delete_tombstones_each_recreated_definition() {
        ...
        let first_delete = delete_workflow_and_definition_for_owner(
            &pool,
            community,
            workflow_id,
            &owner,
        ...
        assert_eq!(
            ...
            WorkflowLifecycleDeleteResult {
                ...
                affected_channel_ids: vec![channel_id],
        ...
        // A replayed tombstone between the recreated definition and a newer
        // duplicate may retire only the older definition. The current workflow
        // row and newer live head must survive.
        let stale_delete = delete_workflow_and_definition_for_owner(
            &pool,
            community,
            workflow_id,
            &owner,
        ...
        .await
        .expect("apply stale workflow tombstone");
        assert_eq!(
            stale_delete,
            WorkflowLifecycleDeleteResult {
                workflow_deleted: false,
        ...
        get_workflow(&pool, community, workflow_id)
            .await
            .expect("newer workflow survives stale tombstone");
        assert!(
            crate::event::get_event_by_id(&pool, community, &second_event_id)
                .await
                .expect("query stale definition")
                .is_none(),
            "the tombstone may retire the definition at its timestamp"
        );
        assert!(
            ...
            .await
            .expect("query newer definition")
            .is_some(),
            "a newer definition must survive a stale tombstone"
        );

        // A current tombstone deletes the remaining definition and workflow
        // row by their authoritative NIP-33 coordinate.
        let second_delete = delete_workflow_and_definition_for_owner(
            &pool,
            community,
            workflow_id,
            &owner,
        ...
        let orphan_cleanup = delete_workflow_and_definition_for_owner(
            &pool,
            community,
            workflow_id,
            &owner,
