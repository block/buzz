...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        let channel_invites = migrations[25].sql.as_str();
        assert!(channel_invites.contains("ADD COLUMN channel_id UUID"));
        assert!(channel_invites.contains("FOREIGN KEY (community_id, channel_id)"));
        assert!(channel_invites.contains("REFERENCES channels (community_id, id)"));
        ...
        assert!(desired_schema.contains("channel_id  UUID"));
        assert!(desired_schema.contains("CREATE INDEX relay_invites_channel_idx"));
    }
