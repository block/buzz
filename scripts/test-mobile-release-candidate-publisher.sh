...
case "${1:-}:${2:-}" in
  api:repos/SaledaAI/saleda-buzz/rulesets/14378754)
    case "$*" in
    ...
    ;;
  api:repos/SaledaAI/saleda-buzz/git/ref/heads/main) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:repos/SaledaAI/saleda-buzz/commits/*) printf '%s\n' "$GH_TARGET_SHA" ;;
  api:--paginate)
    [[ "$3" == "repos/SaledaAI/saleda-buzz/git/matching-refs/tags/mobile-v1.2.3-rc." ]]
    printf '%s' "${GH_EXISTING_REFS:-}"
    ;;
  api:--method)
    endpoint="$4"
    case "$endpoint" in
      repos/SaledaAI/saleda-buzz/git/tags)
        record "$*"
        printf '%s\n' "$GH_TAG_OBJECT_SHA"
        ;;
      repos/SaledaAI/saleda-buzz/git/refs)
        record "$*"
        ;;
      *) exit 2 ;;
    esac
    ;;
  api:repos/SaledaAI/saleda-buzz/git/ref/tags/mobile-v1.2.3-rc.*)
    if [[ "$*" == *'.object.type'* ]]; then
    ...
    ;;
  api:repos/SaledaAI/saleda-buzz/git/tags/*)
    if [[ "$*" == *'.object.type'* ]]; then
...
export GITHUB_REPOSITORY=SaledaAI/saleda-buzz
export RELEASE_TAG_RULESET_ID=14378754
