...
# `git.exe` plus `jq`/`curl`/full `sed`/`awk`/`grep`/`find` — so a bare host gets a
# real dev env, not just bash + coreutils. We do NOT trim INSIDE the tree: bash and
# git load `msys-2.0.dll` and other libraries lazily, and load-bearing pieces
# (terminfo, gawk libs in `usr/`; git templates, certs, DLLs in `mingw64/`) live
# alongside the docs, so a hand-trimmed copy can pass an existence check yet fail
# mid-command with a cryptic error — exactly the bug class this avoids. The
# retained runtime is the untouched, complete closure git-for-windows maintains.
#
...
rm -rf "$tmp_dir"
trap - EXIT
# Assert both load-bearing entry points landed: the launcher bash we resolve at
# runtime, and git.exe in mingw64/ (the binary the whole restore exists to ship).
# A stale or partial extract that lacks either must fail the gate, not write the
# marker — otherwise the idempotency skip would lock in a broken tree.
for required in bin/bash.exe mingw64/bin/git.exe; do
...
touch "$STAGE_MARKER"
echo "PortableGit full toolchain staged at $GIT_BASH_DIR (bash + git + coreutils)"
