...
JSON
git -C "$tmp" add .
git -C "$tmp" commit -qm 'chore(release): release Zorro Desktop version 1.0.0'
...
cat > "$mock_bin/gh" <<GH
#!/usr/bin/env bash
[[ "\$1" == api && "\$2" == "repos/SaledaAI/saleda-buzz/commits/$prior_candidate/pulls" ]] || exit 90
cat <<JSON
...
(
  cd "$tmp"
  PATH="$mock_bin:$PATH" scripts/desktop_release.py generate 1.0.1 --base "$base" --repo SaledaAI/saleda-buzz
  python3 - <<'PY'
import json
...
PY
  git add .
  git -c user.name=Wes -c user.email=wesbillman@users.noreply.github.com commit -q -s -m 'chore(release): release Zorro Desktop version 1.0.1' -m 'Co-authored-by: Test Automation <test@example.com>'
  PATH="$mock_bin:$PATH" scripts/desktop_release.py validate --version 1.0.1 --repo SaledaAI/saleda-buzz
  ...
  if PATH="$mock_bin:$PATH" scripts/desktop_release.py validate --version 1.0.1 --repo SaledaAI/saleda-buzz >/dev/null 2>&1; then
...
module.previous_release("1.0.1-beta", "SaledaAI/saleda-buzz", allow_target_sha=candidate)
PY
...
for invalid_version in 1.0.0 0.9.9; do
  if (cd "$tmp" && PATH="/usr/bin:/bin" scripts/desktop_release.py generate "$invalid_version" --base "$base" --repo SaledaAI/saleda-buzz) >/dev/null 2>&1; then
...
cat > "$mock_bin/gh" <<GH
#!/usr/bin/env bash
[[ "\$1" == api && "\$2" == "repos/SaledaAI/saleda-buzz/commits/$production_tag/pulls" ]] || exit 90
printf '%s\n' '[{"merged_at":"2026-01-01T00:00:00Z","merge_commit_sha":"$production_tag","head":{"sha":"$prior_candidate"}}]'
GH
chmod +x "$mock_bin/gh"
(cd "$migration" && PATH="$mock_bin:$PATH" scripts/desktop_release.py generate 1.0.1 --base "$migration_base" --repo SaledaAI/saleda-buzz)
...
(cd "$initial" && scripts/desktop_release.py generate 0.1.0 --base "$root_sha" --repo SaledaAI/saleda-buzz)
