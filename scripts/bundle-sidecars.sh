...
if [[ "$TARGET" != *windows* ]]; then
    ...
    BUILD_HINT="cargo build --release -p buzz-acp -p buzz-backend-kubernetes -p buzz-dev-mcp -p git-credential-nostr -p buzz-cli"
else
    BUILD_HINT="cargo build --release -p buzz-acp -p buzz-dev-mcp -p git-credential-nostr -p buzz-cli"
fi
