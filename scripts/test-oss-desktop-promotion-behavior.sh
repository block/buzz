...
write_manifest() {
  ...
  jq -n --arg version "$version" --arg signature "$signature" --arg base "https://github.com/SaledaAI/saleda-buzz/releases/download/desktop-v${base_version}" '{
...
run_case() {
  ...
  set +e
  output="$(PATH="$tmp/bin:$PATH" GITHUB_REPOSITORY=SaledaAI/saleda-buzz \
