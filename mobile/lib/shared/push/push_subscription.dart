...
///
/// Workstream A has no lease publisher, so snapshots use [desired] authority.
/// A later publisher must persist an observed [accepted] lease before changing
/// authority. Keeping both sets makes relay rejection, clamping, or expiry
/// detectable instead of assuming the desired lease was accepted unchanged.
  ...
  factory BuzzPushLeaseSubscriptionState.fromJson(Map<String, dynamic> json) {
    ...
    return switch (authority) {
      ...
      'accepted' => throw const FormatException(
        'Accepted push authority requires accepted subscriptions.',
      ),
