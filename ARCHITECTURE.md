...
**Workflow-to-agent authority:** `send_message` output is a kind `9` event signed by the relay identity advertised as NIP-11 `self`. The event carries exactly one `["buzz:workflow", "true"]` marker, one `["workflow-owner", "<64-hex-pubkey>"]` claimed principal, and one `["workflow-definition", "<kind-30620-event-id>", "<step-id>"]` provenance tag. `p` tags remain attribution and mention/wake routing only and never grant authority.

An ACP harness evaluates a workflow message under `workflow-owner` only after two layers verify. First, the delivery envelope must be a valid kind `9` signed by the configured endpoint's NIP-11 `self`, with one exact workflow marker and unambiguous, syntactically valid owner/definition tags. Second, ACP fetches the referenced kind `30620` event and independently verifies its signature and exact event id, requires its author to equal `workflow-owner`, requires its sole `h` channel to equal the delivered message channel, finds the referenced `send_message` step, and matches the delivered content against that owner-signed text (exactly for literal text; by ordered literal skeleton for runtime templates). Missing NIP-11 identity, malformed or duplicate provenance, fetch failure, invalid signature, author/channel/step/content mismatch, a non-relay signer, or a bare owner `p` tag fails closed to ordinary author authorization. The derived workflow owner is then subject to the same `respond_to` policy and DM hardening as a directly authored message.

**Partial rollout:** a new relay paired with an old ACP emits the additional definition tag but retains the old envelope, so delegation continues under the old ACP's relay-rooted checks until that ACP upgrades. An old relay paired with a new ACP omits the mandatory definition reference, so workflow delegation fails closed and agents ignore those workflow messages under owner-only policy; ordinary messages are unaffected. Upgrade ACP promptly after the relay to close the relay-key authority gap, and do not treat a mixed fleet as fully hardened.
