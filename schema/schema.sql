...
-- NIP-CM channel-wide mentions (@channel): one row per notifying event, NOT
-- one per member — the mentions feed resolves recipients at read time by
-- joining channel_members. @here is deliberately absent (live-only, never
-- persisted), which is why `mode` is constrained to 'channel'.
