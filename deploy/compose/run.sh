...
case "${1:-help}" in
  ...
  backup-hint)
    ...
    ;;
  add-member)
    docker compose exec relay /usr/local/bin/buzz-admin add-member --pubkey "${2:?Usage: ./run.sh add-member <npub-or-hex> [--role member|moderator|admin]}" "${@:3}"
    ;;
  remove-member)
    docker compose exec relay /usr/local/bin/buzz-admin remove-member --pubkey "${2:?Usage: ./run.sh remove-member <npub-or-hex> [--role member|moderator|admin]}" "${@:3}"
    ;;
  list-members)
...
Commands:
  ...
  add-member <npub-or-hex> [--role member|moderator|admin]
                Add a relay member (default role: member)
  remove-member <npub-or-hex> [--role member|moderator|admin]
