...
fn script_src_stays_free_of_unsafe_inline_and_eval() {
    ...
    // sha256 hashing, so neither escape hatch is ever needed here. Note: the
    // boot background was previously an inline <style>, but it was moved to
    // public/boot.css (a linked stylesheet) so that Tauri does not also hash
    // style-src — a hash in style-src would override 'unsafe-inline' and block
    // TipTap's runtime stylesheet injection in packaged builds.
