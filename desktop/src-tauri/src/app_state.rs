...
fn identity_from_env() -> Option<Keys> {
    match std::env::var("BUZZ_PRIVATE_KEY") {
        Ok(nsec) => match Keys::parse(nsec.trim()) {
            Ok(keys) => Some(keys),
            Err(error) => {
                eprintln!("zorro-desktop: invalid BUZZ_PRIVATE_KEY: {error}");
                None
            }
        },
        Err(std::env::VarError::NotUnicode(_)) => {
            eprintln!("zorro-desktop: BUZZ_PRIVATE_KEY contains invalid UTF-8");
            None
        }
...
pub fn build_app_state() -> AppState {
    ...
    let (keys, identity_storage) = match identity_from_env() {
        Some(keys) => {
            eprintln!(
                "zorro-desktop: configured identity pubkey {}",
    ...
    pub fn signing_keys(&self) -> Result<Keys, String> {
                ...
                .load(std::sync::atomic::Ordering::Acquire)
        {
            return Err("identity is in recovery mode; event signing is disabled \
                 until the identity is restored and Zorro is relaunched"
                .to_string());
        }
        self.keys
            .lock()
    ...
    match store.probe(IDENTITY_KEY_NAME) {
        KeyringProbe::Present => {
            if let Some(nsec) = store.load(IDENTITY_KEY_NAME)? {
                match Keys::parse(nsec.trim()) {
                    Ok(keyring_keys) => {
                        eprintln!(
                            "zorro-desktop: persisted identity pubkey {}",
                        ...
                        if legacy_path.exists() {
                            match load_key_file(legacy_path) {
                                ...
                                {
                                    eprintln!(
                                        "zorro-desktop: identity.key differs from keyring; \
                                    ...
                                    let storage = if let Err(e) = persist_identity_to_keyring(
                                        ...
                                        eprintln!(
                                            "zorro-desktop: keyring adoption of identity.key \
                                ...
                                Err(e) => {
                                    eprintln!(
                                        "zorro-desktop: leftover identity.key is corrupt ({e}); \
                        ...
                        if !legacy_path.exists() && !migration_marker_path(data_dir).exists() {
                            ...
                            {
                                eprintln!(
                                    "zorro-desktop: keyring present but marker missing; \
        ...
        KeyringProbe::ReachableButEmpty => {
            ...
            if legacy_path.exists() {
                ...
                eprintln!(
                    "zorro-desktop: identity lost — keyring was empty despite migration marker; \
        ...
        KeyringProbe::Unreachable => {
            ...
            if !legacy_path.exists() && migration_marker_path(data_dir).exists() {
                let ephemeral = Keys::generate();
                eprintln!(
                    "zorro-desktop: keyring unreachable but migration marker present; \
...
fn recover_from_keyring(
    ...
    eprintln!("zorro-desktop: corrupt keyring nsec ({error}); clearing, recovering from file");
    if let Err(e) = store.delete(IDENTITY_KEY_NAME) {
        eprintln!("zorro-desktop: failed to clear corrupt keyring value: {e}");
    }
    if legacy_path.exists() {
        if let Some(keys) = migrate_identity_file(store, legacy_path, data_dir)? {
            return Ok(ResolvedIdentity {
    ...
    if migration_marker_path(data_dir).exists() {
        let ephemeral = Keys::generate();
        eprintln!(
            "zorro-desktop: identity lost — keyring had corrupt data and no valid identity.key \
...
fn load_file_or_generate(
    ...
    eprintln!(
        "zorro-desktop: generated and saved identity pubkey {}",
        keys.public_key().to_hex()
    );
    Ok(keys)
}
...
fn migrate_identity_file(
    ...
    let keys = match load_key_file(legacy_path) {
        Ok(keys) => keys,
        Err(error) => {
            eprintln!("zorro-desktop: corrupt identity.key during migration ({error}), skipping");
            return Ok(None);
        }
    };
    let nsec = keys
    ...
    if let Err(e) = write_migration_marker(&marker_path) {
        eprintln!(
            "zorro-desktop: keyring import ok but failed to write migration marker ({e}); \
             keeping identity.key so the key is not stranded"
        );
        return Ok(Some(keys));
    }
    if let Err(e) = std::fs::remove_file(legacy_path) {
        eprintln!("zorro-desktop: keyring import ok but failed to delete identity.key: {e}");
    } else {
        eprintln!("zorro-desktop: migrated identity key into OS keyring");
    }
    Ok(Some(keys))
}
...
fn persist_identity_to_keyring(
    ...
    if let Err(e) = write_migration_marker(&marker_path) {
        ...
        if !legacy_path.exists() {
            if let Err(write_err) = save_key_file(legacy_path, keys) {
                eprintln!(
                    "zorro-desktop: keyring ok but marker write failed ({e}) and \
                ...
                eprintln!(
                    "zorro-desktop: keyring ok but marker write failed ({e}); \
    ...
    if legacy_path.exists() {
        if let Err(e) = std::fs::remove_file(legacy_path) {
            eprintln!("zorro-desktop: keyring write ok but failed to delete identity.key: {e}");
        }
    }
...
fn persist_imported_identity_impl(
    ...
    match persist_identity_to_keyring(store, keys, legacy_path, data_dir) {
        ...
        Err(e) => {
            eprintln!(
                "zorro-desktop: keyring write failed during import ({e}), \
...
fn generate_and_persist(
    ...
    if storage == IdentityStorage::SystemKeyring {
        ...
        if let Err(e) = write_migration_marker(&marker_path) {
            eprintln!(
                "zorro-desktop: stored identity in keyring but failed to write migration marker \
...
fn store_key_preferring_keyring(
    ...
    match store.store(IDENTITY_KEY_NAME, &nsec) {
        Ok(()) => Ok(IdentityStorage::SystemKeyring),
        Err(keyring_err) => {
            eprintln!("zorro-desktop: keyring write failed ({keyring_err}), using file fallback");
            save_key_file(legacy_path, keys)?;
            Ok(IdentityStorage::LocalFile)
        }
    }
...
fn cleanup_leftover_identity_file(legacy_path: &std::path::Path) {
    ...
    match std::fs::remove_file(legacy_path) {
        Ok(()) => eprintln!("zorro-desktop: removed leftover identity.key (key is in keyring)"),
        Err(e) => eprintln!("zorro-desktop: failed to remove leftover identity.key: {e}"),
    }
}
...
fn quarantine_corrupt_key(key_path: &std::path::Path, data_dir: &std::path::Path, error: &str) {
    ...
    eprintln!("zorro-desktop: corrupt identity.key ({error}), quarantining to {bad_name}");
