...
/// Persist an imported identity key. Tries the OS keyring first (store →
/// read-back verify → marker → delete any leftover file); if the keyring is
/// unavailable, falls back to the `0o600` identity.key file. Returns `Err`
/// only when both the keyring write and the file fallback fail.
