...
use buzz_core_pkg::channel_mentions::{NotifyMode, NOTIFY_TAG};
...
/// Validate and append the NIP-CM channel-wide mention marker.
///
/// At most one `["notify", mode]` tag per event; the mode is parsed by
/// `buzz_core::channel_mentions` so the desktop and the relay accept exactly
/// the same spellings (lowercase `channel` / `here`).
fn notify_tag(notify: Option<&str>, tags: &mut Vec<Tag>) -> Result<(), String> {
    let Some(raw) = notify else {
        return Ok(());
    };
    let mode = NotifyMode::from_str(raw).map_err(|e| e.to_string())?;
    tags.push(tag(vec![NOTIFY_TAG, mode.as_str()])?);
    Ok(())
}
