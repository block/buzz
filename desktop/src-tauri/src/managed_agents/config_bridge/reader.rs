...
pub(crate) fn read_config_surface(
    ...
    let normalized = NormalizedConfig {
        ...
        provider: build_provider_field(
            ...
            provider_env_var,
            provider_locked.then_some((locked_provider_id, locked_provider_label)),
...
fn build_provider_field(
    ...
    locked_provider: Option<(Option<&str>, Option<&str>)>,
    is_required: bool,
    tiers: &InheritedConfigTiers,
) -> Option<NormalizedField> {
    if let Some((locked_provider_id, locked_provider_label)) = locked_provider {
