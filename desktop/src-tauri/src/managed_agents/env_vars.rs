...
pub(crate) const RESERVED_ENV_KEYS: &[&str] = &[
    ...
    // Authorization trust is derived from the structured command wrapper at
    // the spawn boundary. User env must never opt itself into auto-approval.
    "BUZZ_ACP_TRUST_NXTLINQ_GATEWAY",
