...
pub(crate) const RESERVED_ENV_KEYS: &[&str] = &[
    ...
    // Diagnostic origin and observer generation. Neither is lifecycle
    // authority, but descriptor overrides could forge observer frames.
    "BUZZ_MANAGED_AGENT",
    "BUZZ_MANAGED_AGENT_START_NONCE",
    // Durable runtime identity, state and privileged job configuration are
    // resolved by Desktop. A descriptor must never redirect receipt/state
    // paths, downgrade durability, or choose the executable/workspace roots
