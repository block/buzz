...
pub fn validate_user_env_keys(env_vars: &BTreeMap<String, String>) -> Result<(), String> {
    ...
    if !reserved.is_empty() {
        return Err(format!(
            "the following env vars are reserved by Zorro and cannot be overridden: {}",
            reserved.join(", ")
        ));
    }
...
pub(crate) fn merged_user_env(
    ...
    merged.retain(|k, v| {
        if is_reserved_env_key(k) {
            eprintln!(
                "zorro-desktop: ignoring reserved env var `{k}` from persona/agent overrides"
            );
            return false;
        }
        if !is_well_formed_env_key(k) {
            ...
            eprintln!(
                "zorro-desktop: ignoring malformed env var key `{}` from persona/agent overrides",
        ...
        if v.contains('\0') {
            ...
            eprintln!(
                "zorro-desktop: ignoring env var `{k}` with NUL byte in value"
            );
            return false;
        }
        if v.len() > MAX_ENV_VALUE_BYTES {
            eprintln!(
                "zorro-desktop: ignoring env var `{k}` with oversize value ({} bytes > {MAX_ENV_VALUE_BYTES})",
                v.len()
            );
            return false;
        }
