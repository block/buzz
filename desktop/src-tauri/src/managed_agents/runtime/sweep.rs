...
pub(crate) fn sweep_untracked_bundle_harnesses(skip_pids: &[u32]) {
    ...
    eprintln!(
        "zorro-desktop: sweep_untracked_bundle_harnesses: reaping {} stale harness process(es) {:?} (exe: {})",
        to_kill.len(),
        to_kill,
        harness_exe.display(),
    );
...
mod tests {
    ...
    fn strip_deleted_suffix_removes_kernel_suffix() {
        ...
        let p = PathBuf::from("/Applications/Zorro.app/Contents/MacOS/buzz-acp (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/Applications/Zorro.app/Contents/MacOS/buzz-acp")
        );
    }

    #[test]
    fn strip_deleted_suffix_leaves_normal_path_unchanged() {
        let p = PathBuf::from("/Applications/Zorro.app/Contents/MacOS/buzz-acp");
    ...
    const BUNDLE_HARNESS: &str = "/Applications/Zorro.app/Contents/MacOS/buzz-acp";
    ...
    fn child_of_tracked_parent_not_directly_targeted() {
        ...
        let snapshots = vec![snap(1004, "/Applications/Zorro.app/Contents/MacOS/goose")];
        let result =
            select_untracked_bundle_harnesses(&snapshots, &PathBuf::from(BUNDLE_HARNESS), &[]);
        assert!(result.is_empty());
    }
    ...
    fn deleted_suffix_stripped_path_matches_expected() {
        ...
        let raw = PathBuf::from("/Applications/Zorro.app/Contents/MacOS/buzz-acp (deleted)");
