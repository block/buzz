...
mod managed_agent;
pub use heartbeat_preflight::HeartbeatPreflightDesignation;
pub(crate) use heartbeat_preflight::{
    apply_heartbeat_preflight_update, validate_heartbeat_preflight_configuration,
};
pub use managed_agent::{ManagedAgentProcess, ManagedAgentSummary};
...
pub struct ManagedAgentRecord {
    ...
    /// Owner-authoritative heartbeat-preflight designation. Presence is the
    /// durable must-check latch; the policy itself is re-read and re-hashed by
    /// the harness before every heartbeat. Missing fields on legacy records
    /// deserialize as `None` and preserve their existing ungated behavior.
    #[serde(default, skip_serializing_if = "Option::is_none")]
