...
#[test]
fn collaboration_binding_changes_snapshot() {
    let rec = record();
    let mut edited = record();
    edited.collaboration_role = "main".into();
    edited.managed_project_ids = vec!["demo".into()];
    edited.collab_helper_root = "/tmp/helper".into();
    assert_ne!(
        snapshot(&rec, &[], &[], "wss://ws.example", &Default::default()),
        snapshot(&edited, &[], &[], "wss://ws.example", &Default::default())
    );
