...
///
/// Derived from `PRESET_HARNESSES` plus the tier-1 runtime IDs.
fn builtin_ids() -> impl Iterator<Item = &'static str> {
    const TIER1: &[&str] = &["goose", "claude", "codex", "antigravity", "buzz-agent"];
...
mod tests {
    ...
    fn builtin_ids_are_rejected() {
        ...
        for id in &["goose", "claude", "codex", "antigravity", "buzz-agent"] {
