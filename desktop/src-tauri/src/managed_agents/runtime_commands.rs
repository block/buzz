...
async fn probe_agent_relay_access(
    ...
    // Probe over the requested authority, not the canonical key form — the
    // Host header picks the community, and the probe must hit the same one
    // the child will join (see `child_connect_relay_url`).
        ...
        for probe in probes {
            match probe {
                Ok((record, key, requested)) => {
                    // Configured spelling, not `key.relay_url` — Host-derived
                    // tenancy (see `child_connect_relay_url`).
                    match start_pair(
                        record.pubkey.clone(),
                        requested.clone(),
                        true,
