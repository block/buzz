...
pub fn spawn_agent_child(
    ...
    // from the SAME `effective_cfg` the spawn-config snapshot reads, so env
    ...
    // onto a child running the OLD one, silently suppressing the badge. The
    // prompt/model/provider come from the same `effective_cfg` resolve the
    // launch contract was built from, and team instructions from the same
    // helper `resolve_launch_spec` calls — stamp and contract cannot disagree.
    ...
    let spawn_config = super::spawn_snapshot::SpawnConfigSnapshot::from_inputs(
        super::spawn_snapshot::SpawnConfigInputs {
            ...
            provider: effective_cfg.provider.value.as_deref(),
        },
    );
