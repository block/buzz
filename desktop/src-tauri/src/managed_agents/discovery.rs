...
const KNOWN_ACP_RUNTIMES: &[KnownAcpRuntime] = &[
    ...
    // Grok Build speaks ACP natively over stdio (`grok agent stdio`) — no
    // separate *-acp npm adapter. Auth: `grok login` or XAI_API_KEY. For
    // managed/headless agents we default `--always-approve` so tool calls do
    // not block on a TTY prompt. Registry uses ONLY the `grok` binary; bare
    // `agent` must not appear in `commands` (Cursor disambiguates that shim).
    KnownAcpRuntime {
        id: "grok",
        label: "Grok Build",
        commands: &["grok"],
        ...
        adapter_install_commands: &[],
        cli_install_instructions_url: "https://docs.x.ai/developers/guides/grok-cli",
        adapter_install_instructions_url: "",
