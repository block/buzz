...
// Its only test is `#[cfg(unix)]`, so on Windows this re-export is unused and
// `-D warnings` rejects it. Same reason four other names were dropped from
// `discovery/tests.rs`'s import list.
#[cfg(all(test, unix))]
