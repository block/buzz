...
#[cfg(test)]
// Every caller is a `#[cfg(unix)]` test, so on Windows this is dead code and
// `-D warnings` rejects it. Fork-only divergence: upstream's CI does not build
// this crate for Windows, ours does.
