...
fn minimal_record() -> ManagedAgentRecord {
    ManagedAgentRecord {
        ...
        env_vars: {
            ...
            // Value MUST NOT appear; key name appears as environment scaffolding.
...
#[test]
fn secret_exclusion_env_var_values_absent() {
    let record = minimal_record();
    let json = snapshot_json_string(&record);
    assert!(
        !json.contains("secret123"),
        "env var values must not appear in snapshot"
    );
    assert!(
...
// ── Environment scaffolding tests ────────────────────────────────────────────
//
// Key NAMES travel (sorted, values never) so import can pre-create blank
// entries; reserved/malformed keys and empty maps are excluded.

#[test]
fn environment_keys_serialized_sorted_values_excluded() {
    let mut record = minimal_record();
    record.env_vars = BTreeMap::from([
        ("ZEBRA_TOKEN".to_string(), "z-secret".to_string()),
        (
            "ALPHA_ENDPOINT".to_string(),
    ...
    assert_eq!(
        ...
        "environment must carry sorted key names only"
    );
    let json = snapshot_json_string(&record);
    assert!(
        !json.contains("z-secret") && !json.contains("https://a.example"),
        "environment values must not appear in snapshot"
    );
}

#[test]
fn environment_excludes_reserved_and_malformed_keys() {
    let mut record = minimal_record();
    record.env_vars = BTreeMap::from([
        ("BUZZ_PRIVATE_KEY".to_string(), "forged".to_string()), // reserved
        ("BAD KEY".to_string(), "v".to_string()),               // malformed
        ("VALID_KEY".to_string(), "v".to_string()),
    ]);
    let snapshot = build_snapshot(&record, MemoryLevel::None, vec![], None);
    assert_eq!(snapshot.definition.environment, vec!["VALID_KEY"]);
}

#[test]
fn environment_field_absent_when_no_env_vars() {
    let mut record = minimal_record();
    record.env_vars = BTreeMap::new();
    let json = snapshot_json_string(&record);
    assert!(
        !json.contains("environment"),
        "empty environment must be omitted from the manifest"
    );
}

#[test]
fn decode_manifest_without_environment_field_succeeds() {
    // Backward compatibility: manifests produced before environment
    // scaffolding existed (or by third-party v1 producers) omit the field.
    let json = r#"{
        "format": "buzz-agent-snapshot",
        "version": 1,
        "definition": { "name": "Legacy Agent" },
        "profile": { "displayName": "Legacy Agent" },
        "memory": { "level": "none" }
    }"#;
    let parsed = decode_snapshot_json(json.as_bytes()).unwrap();
    assert!(parsed.definition.environment.is_empty());
}

#[test]
fn validate_rejects_oversized_environment() {
    let keys: Vec<String> = (0..=MAX_SNAPSHOT_ENV_KEYS)
        .map(|i| format!("KEY_{i}"))
        .collect();
    let manifest = serde_json::json!({
        "format": "buzz-agent-snapshot",
        "version": 1,
        "definition": { "name": "Hostile", "environment": keys },
        "profile": { "displayName": "Hostile" },
        "memory": { "level": "none" }
    });
    let bytes = serde_json::to_vec(&manifest).unwrap();
    let err = decode_snapshot_json(&bytes).unwrap_err();
    assert!(
        err.contains("environment"),
        "error must identify the environment section: {err}"
    );
}

#[test]
fn export_manifest_never_serializes_environment_values() {
    // Buzz's own export must never emit the value-hint carrier, even when the
    // record's env values are non-secret: only external producers may fill it.
    let mut record = minimal_record();
    record.env_vars = BTreeMap::from([(
    ...
    let json = snapshot_json_string(&record);
    assert!(
        !json.contains("environmentValues"),
        "Buzz export must not serialize definition.environmentValues"
    );
}

#[test]
fn decode_manifest_accepts_environment_value_hints() {
    let manifest = serde_json::json!({
        "format": "buzz-agent-snapshot",
        "version": 1,
        "definition": {
            "name": "Studio Agent",
            "environment": ["OPENAI_COMPAT_API_KEY", "OPENAI_COMPAT_BASE_URL"],
            "environmentValues": {
                "OPENAI_COMPAT_BASE_URL": "https://app.inloop.studio/api/v1/brains/example"
            }
        },
        "profile": { "displayName": "Studio Agent" },
        "memory": { "level": "none" }
    });
    let bytes = serde_json::to_vec(&manifest).unwrap();
    let parsed = decode_snapshot_json(&bytes).unwrap();
...
#[test]
fn validate_rejects_oversized_environment_values() {
    let values: serde_json::Map<String, serde_json::Value> = (0..=MAX_SNAPSHOT_ENV_KEYS)
        .map(|i| (format!("KEY_{i}"), serde_json::Value::from("v")))
        .collect();
    let manifest = serde_json::json!({
        "format": "buzz-agent-snapshot",
        "version": 1,
        "definition": { "name": "Hostile", "environmentValues": values },
        "profile": { "displayName": "Hostile" },
        "memory": { "level": "none" }
    });
    let bytes = serde_json::to_vec(&manifest).unwrap();
    let err = decode_snapshot_json(&bytes).unwrap_err();
    assert!(
        err.contains("environmentValues"),
        "error must identify the environmentValues section: {err}"
    );
}

#[test]
fn validate_rejects_oversize_single_environment_value() {
    let manifest = serde_json::json!({
        "format": "buzz-agent-snapshot",
        "version": 1,
        "definition": {
            "name": "Hostile",
            "environment": ["BIG_HINT"],
            "environmentValues": {
                "BIG_HINT": "x".repeat(crate::managed_agents::MAX_ENV_VALUE_BYTES + 1)
            }
        },
        "profile": { "displayName": "Hostile" },
        "memory": { "level": "none" }
