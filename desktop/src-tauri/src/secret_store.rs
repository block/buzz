...
impl SecretStore {
    ...
    pub fn delete_all_with_legacy_cleanup(&self) -> Result<(), String> {
        #[cfg(feature = "system-keyring")]
        {
            ...
            for i in 0..index_chunks.max(1).min(MAX_INDEX_CHUNKS) {
...
mod fake_backend {
    ...
    pub fn seed(service: &str, key: &str, value: &str) {
        creds().lock().unwrap_or_else(|e| e.into_inner()).insert(
...
mod tests {
    ...
    fn migration_rolls_back_and_leaves_blob_intact_when_one_write_fails() {
        ...
        assert_eq!(fake_backend::raw(svc, &key3).as_deref(), Some(value3.as_str()));
        assert_eq!(
            SecretStore::with_fake(svc).load_all_readonly().unwrap().unwrap(),
            blob
        );
    ...
    fn migration_rollback_restores_prior_credential_values() {
        ...
        let store = SecretStore::with_fake(svc);
        let err = store.ensure_migrated().unwrap_err();
        assert!(err.contains("index"), "the index write must be the failure: {err}");
    ...
    fn sixteen_agents_plus_identity_all_reach_the_credential_store() {
        ...
        let after = SecretStore::with_fake(svc).load_all_readonly().unwrap().unwrap();
