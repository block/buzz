...
/// Metadata stripping lives in `buzz-image`, shared with the relay's validator
/// and the CLI. Re-exported here so the existing call sites read unchanged.
pub(crate) use buzz_image::sanitize_image_for_upload;
