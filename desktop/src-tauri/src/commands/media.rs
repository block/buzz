...
pub(crate) async fn mint_media_get_auth(
    ...
    // ordinary leased operation (spec L4569-4570). The durable capability is
    // stamped with the LEASE's admission generation, not a re-read one — so a
    // transition that bumps the generation between admission and registration
    // stamps a stale value and the first admit_exercise refuses (fail-closed).
    ...
    let bearer = crate::owner_identity_egress::register_owner_bearer(&lease);
    drop(lease);
    Some((header, bearer))
}
...
async fn do_upload(
    ...
    if should_retry_legacy_upload(resp.status()) {
        // Re-validate before the second transmission: the same signed header is
        // resent, and a transition may have superseded the bearer between the
        // two dispatches. Two transmissions, two validations.
        bearer.admit_exercise()?;
