...
// Retention helpers live in `agents_retention` (file-size ratchet).
pub(crate) use super::agents_retention::{
...
pub(super) async fn start_local_agent_with_preflight(
    ...
    if record_snapshot.ephemeral {
        let grant = crate::managed_agents::load_global_agent_config(app)
            .unwrap_or_default()
            .allow_temp_agent_spawn;
        crate::managed_agents::ephemeral::assert_temp_can_start(
            &record_snapshot.pubkey,
            true,
            grant,
            record_snapshot.expires_at.as_deref(),
        )?;
    }
...
pub async fn create_managed_agent(
    ...
    if input.ephemeral && !input.temp_spawn_canonical {
        return Err("temporary agents must be created via spawn_temp_managed_agent".into());
    }
    let requested_persona_id = if input.ephemeral {
        None
    } else {
        input
            .persona_id
    ...
    let (agent, resolved_avatar_url) = {
        ...
        let temp_locked = if input.ephemeral {
            let global = crate::managed_agents::load_global_agent_config(&app).unwrap_or_default();
            Some(
                crate::managed_agents::ephemeral::recheck_temp_before_insert(
                    &global,
                    &records,
                    &name,
                    input.parent_agent_pubkey.as_deref(),
                    input.channel_id.as_deref(),
                    input.expires_at.as_deref(),
                    input.backend == BackendKind::Local,
                    input.respond_to == Some(crate::managed_agents::RespondTo::Allowlist),
                )?,
            )
        } else {
            None
        ...
        let record = crate::managed_agents::ManagedAgentRecord {
            ...
            start_on_app_launch: if input.ephemeral || input.backend != BackendKind::Local {
                false
            } else {
                input.start_on_app_launch
            },
            auto_restart_on_config_change: !input.ephemeral,
            runtime_pid: None,
            ...
            parent_agent_pubkey: temp_locked.as_ref().map(|x| x.parent_agent_pubkey.clone()),
            expires_at: temp_locked.as_ref().map(|x| x.expires_at.clone()),
            channel_id: temp_locked.as_ref().map(|x| x.channel_id.clone()),
        };
