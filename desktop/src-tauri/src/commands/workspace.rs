...
fn migrate_legacy_retention_into(
    ...
    match crate::managed_agents::retention::migrate_legacy_retention_db(
        ...
        Ok(copied) => {
            eprintln!(
                "zorro-desktop: adopted {copied} legacy retained event(s) into this community"
            )
        }
        Err(error) => eprintln!("zorro-desktop: legacy retention migration failed: {error}"),
    }
}
...
pub async fn apply_workspace(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        if let Some(nest) = nest.as_deref() {
            if let Err(error) = write_persisted_repos_dir(nest, effective_repos_dir.as_deref()) {
                eprintln!("zorro-desktop: persist repos dir failed: {error}");
            }
            if let Err(error) = ensure_repos_symlink(nest, effective_repos_dir.as_deref()) {
                eprintln!("zorro-desktop: repos dir setup failed: {error}");
                let _ = app.emit("repos-dir-error", error);
            }
        }
    ...
    match crate::managed_agents::retention::active_retention_scope(&restore_app, &state) {
        ...
        Err(error) => {
            eprintln!(
                "zorro-desktop: scoped event-sync unavailable after workspace apply: {error}"
            );
        }
    }
    ...
    #[cfg(feature = "mesh-llm")]
    {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if restore_pending {
                ...
                {
                    eprintln!("zorro-desktop: failed to restore Share Compute: {error}");
                }
            }
            ...
            if restore_pending {
                ...
                {
                    eprintln!("zorro-desktop: failed to restore managed agents: {error}");
                }
            }
        });
    }
