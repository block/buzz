...
mod linux {
    ...
    pub fn show(
        ...
        std::thread::spawn(move || {
            ...
            // A default action must be set so the desktop shell registers clickability.
            notification.action("default", "default");

            let handle = match notification.show() {
                Ok(handle) => handle,
                Err(error) => {
                    eprintln!("buzz-desktop: failed to post Linux native notification: {error}");
                    return;
                }
            };

            // Hold the D-Bus connection open until the user acts or dismisses.
...
mod windows {
    ...
    mod tests {
        ...
        fn shortcut_failure_is_propagated() {
            let directory = tempfile::tempdir().expect("temporary programs folder");
            let executable = std::env::current_exe().expect("test executable");
            let bad_path = directory.path().join("not-a-folder");
            std::fs::write(&bad_path, "blocking file").expect("write blocker");
            assert!(ensure_shortcut_in(&bad_path, "Buzz", &executable, "buzz.test").is_err());
        }
    }
}
