...
use super::export_util::{save_bytes_with_dialog, save_json_with_dialog};
use crate::{
    app_state::AppState,
...
///
/// `id` is the agent's `pubkey` (for instantiated agents) or `slug` (for
/// definition-only records). `memory_level` is one of `"none"`, `"core"`,
/// or `"everything"`. `format` is either `"json"` or `"png"`.
///
/// PNG exports with `memory_level != "none"` are rejected with a clear error.
///
...
pub async fn export_agent_snapshot(
    ...
    let record = {
        ...
        // Match by pubkey first (exact), then by slug.
        agents
            .into_iter()
            .find(|a| a.pubkey == id || a.slug.as_deref() == Some(id.as_str()))
            .ok_or_else(|| format!("agent {id:?} not found"))?
    };
    ...
    // ── Fetch memory ─────────────────────────────────────────────────────────
    let memory_entries: Vec<AgentSnapshotMemoryEntry> = if memory_level != MemoryLevel::None {
