...
pub fn availability_from_events(events: Vec<nostr::Event>) -> MeshAvailability {
    if events.is_empty() {
        return MeshAvailability::unavailable("Zorro shared compute status is not published yet");
    }
    let Some(members) = latest_membership_list(&events) else {
        return MeshAvailability::unavailable(
            "Zorro shared compute is waiting for the current member roster",
        );
    };
    ...
    if !saw_valid_status {
        return MeshAvailability::unavailable("Zorro shared compute status is malformed");
    }
    ...
    MeshAvailability {
        reason: if available {
            ...
            Some("no Zorro shared compute serving members are available".to_string())
        },
        models,
        serve_targets,
    }
