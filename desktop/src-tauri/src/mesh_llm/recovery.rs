...
pub(crate) async fn rearm_relay_mesh_for_running_agents(app: &AppHandle) -> Result<(), String> {
    ...
    match recovery {
        ...
        MeshRuntimeRecovery::RestartRequired => {
            if runtime_mode == Some(crate::mesh_llm::MeshNodeMode::Serve) {
                eprintln!(
                    "buzz-mesh: serving ingress failed; restarting Zorro to restore Share Compute without changing roles"
                );
                app.request_restart();
                return Ok(());
            }
            ...
            eprintln!(
                "buzz-mesh: supervised client startup lost its ingress before the SDK exposed a shutdown handle; restarting Zorro"
            );
            app.request_restart();
            return Ok(());
        }
    ...
    for (record, mesh_model_id) in &mesh_records {
        ...
        .await
        {
            ...
            Err(error) => {
                let message = format!(
                    "{MESH_REARM_ERROR_SENTINEL}Zorro shared compute offline — failed to re-arm local ingress for this agent: {error}"
                );
