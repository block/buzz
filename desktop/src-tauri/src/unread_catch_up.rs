    ...
    for (item, discovered) in fetched.into_iter().zip(discoveries) {
        ...
        for event in item.events {
            ...
            let high_priority = item.channel.channel_type == "dm"
                ...
                // entire distinction from `@here`, which deliberately does NOT
                // appear here: an `@here` you missed while offline must not
                // come back as urgent, because you were not "here".
                //
                // Fork-only. Upstream has no mention-scope tag, so this branch
                // is invisible to it and will be dropped by any careless
                // catch-up merge. See `events/message_tags.rs`.
                || has_exact_tag(&event.tags, "mention-scope", "channel");
