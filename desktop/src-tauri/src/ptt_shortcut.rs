...
/// Build the global-shortcut plugin with the PTT press/release handler.
///
/// Extracted from `run()` to keep `lib.rs` under the line-count ratchet.
...
/// before any test runs. The call site is therefore gated with
/// `#[cfg(not(test))]`.
///
/// The `ptt_press_gen` counter prevents press→release→press within 200 ms
/// from having the first release clobber the second press: each press bumps
/// the generation, and the delayed release task bails if the generation has
/// changed when it wakes.
#[cfg(not(test))]
pub fn build_plugin() -> impl tauri::plugin::Plugin<tauri::Wry> {
    ...
    // (i.e. no new press happened during the delay).
    let ptt_press_gen = Arc::new(AtomicU64::new(0));

    tauri_plugin_global_shortcut::Builder::new()
        .with_handler(move |app, _shortcut, event| {
            let state = match app.try_state::<crate::app_state::AppState>() {
                Some(s) => s,
                None => return,
            };
