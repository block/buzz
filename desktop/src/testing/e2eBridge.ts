...
async function handleDiscoverAcpRuntimes(
  ...
  const defaultCatalog: RawAcpRuntimeCatalogEntry[] = [
    ...
    },
    {
      ...
      install_hint:
        "Buzz requires Kiro CLI; the desktop app alone is not enough.",
      install_instructions_url: "https://kiro.dev/docs/cli/installation/",
      can_auto_install: true,
      requires_external_cli: true,
      underlying_cli_path: null,
      node_required: false,
      auth_status: { status: "unknown" },
      login_hint: "Run `kiro-cli login` to authenticate.",
    },
    {
      id: "buzz-agent",
      label: "Buzz Agent",
