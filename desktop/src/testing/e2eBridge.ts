...
type E2eConfig = {
  ...
  mock?: {
    ...
    // When true, `get_identity` returns `locked: true` until `import_identity` or
    // `persist_current_identity` is called. Drives the keyring-locked screen in tests.
  ...
  const handleMockCommand = async (command: string, payload: unknown) => {
    ...
    switch (command) {
      ...
      case "persist_current_identity": {
        // Persist the ephemeral key: clear both recovery flags and return the
        // current identity with lost: false, locked: false. Pubkey stays the same.
