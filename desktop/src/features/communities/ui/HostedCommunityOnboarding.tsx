      ...
      if (response.error) {
        throw new Error(
          hostedCommunityErrorMessage(
            ...
            "Could not connect the Mesh identity.",
          ),
        );
      }
      ...
      if (released.error) {
        throw new Error(
          hostedCommunityErrorMessage(
            ...
            "Could not disconnect the account's previous Mesh identity.",
          ),
        );
      }
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ...
            : hostedCommunityErrorMessage(
                ...
                "Could not connect this device's Mesh identity.",
              ),
        );
      }
      ...
      if (available.error || !available.available) {
        ...
        throw new Error(
          hostedCommunityErrorMessage(
            ...
            "That Mesh address is already taken.",
          ),
        );
      }
        ...
        <div className="mx-auto flex w-full max-w-sm flex-col items-center py-2 text-center">
          ...
          {!auth ? (
            <>
              ...
              {/* Quiet breadcrumb: Mesh itself is open source; this hosted
              ...
              <p className="mt-6 w-full border-t border-foreground/10 pt-4 text-xs leading-5 text-foreground/45">
                Mesh is open source. Builderlab hosts the relay for this
                account.
              </p>
            </>
          ) : !identity ? (
            <>
              <DialogTitle className="text-xl font-medium text-foreground">
                Finish connecting Mesh
              </DialogTitle>
              <DialogDescription className="mt-2 text-sm leading-6 text-foreground">
                Your Builderlab account
                {auth.email ? ` (${auth.email})` : ""} is ready. Connect this
                device’s Mesh identity to finish setup. Your private key stays
                on this device.
              </DialogDescription>
              {errorBox ? <div className="mt-5 w-full">{errorBox}</div> : null}
              <Button
            ...
            <>
              <DialogTitle className="text-xl font-medium text-foreground">
                This account uses a different Mesh identity
              </DialogTitle>
              <DialogDescription className="mt-2 text-sm leading-6 text-foreground">
                This account is connected to another Mesh identity. Reconnect
    ...
    <div className="flex min-h-[calc(100dvh-15.625rem)] w-full max-w-[920px] flex-col items-center text-center">
      ...
      <p className="mx-auto mt-2 max-w-[560px] text-sm leading-6 text-foreground">
        {hasCommunities
          ...
          : "Claim a Mesh address to get started."}
      </p>
