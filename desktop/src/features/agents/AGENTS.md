...
18. **Market buying is opt-in managed-agent behavior, not a detached watcher process.** A truthy `BUZZ_MARKET_BUYER` in the effective definition/instance env lets `managed_agents/market_buyer.rs` configure the existing ACP heartbeat at spawn. Keep the prompt and interval Desktop-owned so runtime state and process lifetime stay truthful. The prompt gives the model relevance judgment but requires canonical channel verification and a signed channel `response`; joining is never treated as buying. Fixed-price purchases remain one response per listing. Ascending auctions require a positive private `BUZZ_MARKET_MAX_SATS`; bidders repeatedly submit only the minimum valid increment when outbid, never reveal their ceiling, and stop at it. Lazy harnesses must preserve a due heartbeat as wake-worthy work in `buzz-acp` rather than skipping it while the worker pool sleeps.
