...
export function useRelayAgentsQuery(options?: { enabled?: boolean }) {
  ...
  return useQuery({
    ...
    // Relay agent profiles (kind:10100/30177) are near-static and the backing
    ...
    // re-pulls the full set app-wide. Live profile and managed-definition
    // updates invalidate this query through `useAgentsDataRefresh`; keep the
    // relaxed poll as a backstop for events missed while disconnected, and
    // pause it while backgrounded.
    refetchInterval,
...
export function useCodexTasksQuery(options?: { enabled?: boolean }) {
  return useQuery({
    enabled: options?.enabled ?? true,
