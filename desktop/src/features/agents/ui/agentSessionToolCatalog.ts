...
export function getBuzzToolInfo(title: string): BuzzToolInfo | null {
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? "Reads workflow state from Mesh."
        : "Updates workflow state in Mesh.",
  ...
  if (
    ...
    return {
      icon: Hash,
      label: isRead
        ? "Reads channel context from the Mesh relay."
        : "Changes channel state in the Mesh relay.",
      tone: isWrite ? "write" : "read",
    };
  }
  if (
    ...
    return {
      icon: Users,
      label: isRead
        ? "Reads Mesh identity or presence data."
        : "Updates Mesh identity or membership data.",
  ...
  if (name.includes("search") || name === "get_feed") {
    return {
      icon: Search,
      label: "Searches relay-visible Mesh history.",
      tone: "read",
    };
  }
  if (
    ...
    return {
      icon: Send,
      label: "Publishes relay-visible Mesh activity.",
      tone: "write",
    };
  }
  ...
  return {
    icon: MessageSquare,
    label: isRead ? "Reads from Mesh." : "Writes to Mesh.",
    tone: isWrite ? "write" : "read",
  };
}
