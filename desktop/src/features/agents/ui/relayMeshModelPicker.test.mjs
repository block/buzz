...
test("Zorro shared compute maps persisted auto to Default and hides custom input", () => {
...
test("Zorro shared compute fallback is Default auto while normal providers remain unchanged", () => {
...
test("Zorro shared compute keeps Default auto when discovery is empty", () => {
  const state = relayMeshModelPickerState({
    discoveredOptions: [],
    fallbackOptions: fallback,
    isCustomEditing: false,
...
test("Zorro shared compute dropdown contains Default plus live models and no custom escape hatch", () => {
