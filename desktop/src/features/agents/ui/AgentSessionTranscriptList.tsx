...
export function AgentSessionTranscriptList({
  ...
  // Streaming/duration hints for the conversation variant's thought disclosure.
    ...
    >
      <div
        ...
        className={cn(
          ...
          isCompactPreview ? "gap-1" : isConversation ? "gap-8" : "gap-4",
          // Focus mode reads as a document: center the column and cap it at a
          // comfortable measure so long agent prose does not run edge to edge.
          isConversation && "mx-auto max-w-3xl",
          autoTail && "pb-4",
          contentContainerClassName,
        )}
        data-transcript-variant={isConversation ? "conversation" : undefined}
...
function TranscriptDisplayBlockView({
  ...
  return (
    <div
      className={cn(
        "flex flex-col",
        isCompactPreview ? "gap-2.5" : isConversation ? "gap-3" : "gap-4",
      )}
