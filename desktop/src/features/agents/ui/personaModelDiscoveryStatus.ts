...
export function formatModelDiscoveryErrorStatus(
  ...
  if (provider.trim() === "relay-mesh") {
    if (message.includes("waiting for the current member roster")) {
      return {
        message:
          "Mesh is waiting for the relay's member roster. Try again shortly; if this persists, check the relay's membership configuration.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute is not available in this build")) {
      return {
        message:
          "This version of Mesh cannot use shared compute. Update Mesh or choose another provider.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute status is malformed")) {
      return {
        message:
          "Mesh received an invalid shared compute status. Check the member machine, then try again.",
        tone: "warning",
      };
    }
    ...
    return {
      message:
        "Mesh couldn't check shared compute through the relay. Check your relay connection and try again.",
      tone: "warning",
    };
  }
