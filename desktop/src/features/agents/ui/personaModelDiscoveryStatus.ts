...
function isEmptySharedComputeError(message: string): boolean {
  ...
  return (
    ...
    normalized.includes("no zorro shared compute serving members") ||
    normalized.includes("no live buzz shared compute models") ||
    normalized.includes("no live zorro shared compute models") ||
...
export function formatModelDiscoveryErrorStatus(
  ...
  if (provider.trim() === "relay-mesh") {
    if (message.includes("waiting for the current member roster")) {
      return {
        message:
          "Zorro is waiting for the relay's member roster. Try again shortly; if this persists, check the relay's membership configuration.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute is not available in this build")) {
      return {
        message:
          "This version of Zorro cannot use shared compute. Update Zorro or choose another provider.",
        tone: "warning",
      };
    }
    ...
    if (message.includes("shared compute status is malformed")) {
      return {
        message:
          "Zorro received an invalid shared compute status. Check the member machine, then try again.",
        tone: "warning",
      };
    }
    ...
    return {
      message:
        "Zorro couldn't check shared compute through the relay. Check your relay connection and try again.",
      tone: "warning",
    };
  }
