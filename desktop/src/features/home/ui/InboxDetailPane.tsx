...
function InboxMessageDetailPane({
      ...
      >
        <div className="max-w-sm">
          ...
          <p className="mt-4 text-base font-semibold">
            {t("home.inbox.detail-empty-title")}
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("home.inbox.detail-empty-hint")}
          </p>
        </div>
      </section>
    );
  ...
  const contextLabel = isThreadContext
    ? isDirectMessage
      ? t("home.inbox.thread-with", { name: item.senderLabel })
      : channelContextName
        ? t("home.inbox.thread-in", { channel: channelContextName })
        : t("channels.thread.label")
    : isDirectMessage
      ? t("home.inbox.dm-with", { name: item.senderLabel })
      : channelContextName
        ? t("home.inbox.message-in", { channel: channelContextName })
  ...
  const openContextLabel = isThreadContext
    ? t("home.inbox.open-full-thread")
    : isDirectMessage
      ? t("home.inbox.open-conversation")
      : t("home.inbox.open-in-channel");
          ...
          <div className="px-5 py-2">
            <div className="flex min-h-9 min-w-0 items-center justify-between gap-3">
              ...
              >
                {onBack ? (
                  <Button
                    aria-label={t("home.inbox.back-to-list")}
                    ...
                    >
                      {reopenPending ? (
                        <>
                          ...
                          <span>{t("home.inbox.reopening")}</span>
                        </>
                      ) : (
                        <>
                          <AlertCircle className="h-3.5 w-3.5 shrink-0" />
                          <span>{t("home.inbox.couldnt-reopen")}</span>
                          {contextChannelId ? (
                            <button
          ...
          <div ref={contentRef}>
            {isThreadContextLoading && displayMessages.length <= 1 ? (
              ...
              >
                ...
                <span>{t("home.inbox.loading-context")}</span>
              </div>
            ) : null}
            {hasThreadContextLoadError ? (
              ...
              >
                ...
                <span>{t("home.inbox.context-error")}</span>
              </div>
            ) : null}
          ...
          <div className="pointer-events-auto">
            <MessageComposer
              ...
              placeholder={
                canReply
                  ? isDirectMessage
                    ? t("home.inbox.composer-dm", { name: item.senderLabel })
                    : item.channelLabel
                      ? t("home.inbox.composer-reply-thread", {
                          channel: item.channelLabel,
                        })
                      : t("home.inbox.composer-reply-thread-generic")
                  : (disabledReplyReason ?? t("home.inbox.replies-unavailable"))
              }
