...
test("destroy: aborts in-flight doPublish after fetchOwnBlobBeforePublish resolves", async () => {
  ...
  try {
    ...
    assert.equal(publishCalls.length, 0, "publishEvent must not be called after destroy");
  } finally {
    restore();
    mock.reset();
  }
...
test("revert-fix: fetch failed (error) does not trigger seed-publish via bootstrap", async () => {
  ...
  try {
    ...
    const result = await manager.bootstrap(makeStore({ ch1: { starred: true, updatedAt: 1 } }));
...
test("revert-fix: absent fetch with prior watermark blocks seed-publish via bootstrap", async () => {
  ...
  mock.method(relayClient, "publishEvent", () => Promise.resolve());
  const fw = makeFakeWindow();
  fw.localStorage.setItem(`buzz-sync-watermark.v1:channel-stars:pk-stale:${RELAY_KEY}`, "1700000000");
  const restore = installFakeWindow(fw);
  try {
...
test("revert-fix: relay-A watermark does not suppress first-sync seed on relay-B", async () => {
  ...
  try {
    ...
    const result = await managerB.bootstrap(makeStore({ ch1: { starred: true, updatedAt: 1 } }));
    assert.equal(result.action, "hold");
    assert.ok(managerB.getPendingStarStore() !== null, "first-sync seed on relay B must not be blocked by relay A watermark");
  } finally {
    restore();
    mock.reset();
  }
