...
export function UserProfilePanel({
  ...
  // a card or other surface. Mirrors `subscribeOpenCreateAgent` in AgentsView.
  React.useEffect(() => {
    if (!pubkey) return;
  ...
  // Does THIS desktop hold the agent's seckey (or editable persona)?
  // Gates edit and grants owner access when managed locally.
  const isOwner = resolvedPersona ? true : managedAgentOwner;
  // Is the viewer the declared NIP-OA owner? Right signal for owner-scoped
  // data (activity, memory) — relay routes/decrypts with the owner's own key.
  const isCurrentUserOwner = ownsAuthorAgent(profile, currentPubkey);
  // Viewer sees owner-scoped data if declared-own OR manage locally.
  // Real enforcement is server-side; this only controls which UI we paint.
