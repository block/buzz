    ...
    async (file: File) => {
      ...
      try {
        ...
        if (mountedRef.current)
          setError(t("onboarding.backup.test-error-read"));
        return;
      }
      if (!mountedRef.current) return;
      if (!text.toLowerCase().startsWith("ncryptsec1")) {
        setError(t("onboarding.backup.test-error-not-backup"));
        return;
      }
      setError(null);
      setAttempt("");
      ...
      if (mountedRef.current && requestId === requestRef.current)
        setError(
          err instanceof Error
            ? err.message
            : t("onboarding.backup.test-error-verify"),
        );
    } finally {
      if (mountedRef.current && requestId === requestRef.current)
        ...
        <>
          ...
          <div className="flex items-center justify-center gap-3 pt-2">
            ...
            <Button
              ...
              variant="ghost"
            >
              {t("settings.backup.use-different-file")}
            </Button>
          </div>
        </>
      )}
