...
export function HostedCommunitiesSettingsCard() {
    ...
    if (
      ...
      throw new Error(
        errorMessage(
          ...
          "Could not load the connected Orbit identity.",
        ),
      );
    }
  ...
  const connectIdentity = () =>
    run("Connecting Orbit identity…", async () => {
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not connect the Orbit identity.",
          ),
        );
      }
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            response.error,
            response.correlation_id,
            "Could not unpair the Orbit identity.",
          ),
        );
      }
      setIdentity(null);
      ...
      if (released.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not release the previously connected Orbit identity.",
          ),
        );
      }
      ...
      if (bound.error) {
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ? "This device's Orbit identity is already reserved by another Builderlab account, so it can't be connected here. Sign in with that account, or transfer the identity there first."
            : errorMessage(
                bound.error,
                bound.correlation_id,
                "Could not connect this device's Orbit identity.",
              ),
        );
      }
      ...
      if (availabilityResponse.error || !availabilityResponse.available) {
        ...
        throw new Error(
          errorMessage(
            ...
            "That Orbit address is already taken.",
          ),
        );
      }
      const response = await invoke<CommunityMutationResponse>(
  ...
  return (
    <section className="space-y-6" data-testid="hosted-communities-settings">
      <SettingsSectionHeader
        ...
        description="Orbit works with any relay. This page is only for relay hosting provided by Block — sign in with a Builderlab account to create and manage Block-hosted communities. Builderlab sign-in is used on this page alone."
      />
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border/70 p-5">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Authentication opens in your browser and returns securely to Orbit.
          ...
          {!identity ? (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-5">
              <h3 className="font-medium">
                Link this account to your Orbit identity
              </h3>
              <p className="mt-2 text-sm text-muted-foreground">
                This Builderlab account isn&apos;t linked to an Orbit identity
                yet. Connect this device&apos;s key to create and own
                communities under it — Orbit signs a one-time challenge locally,
            ...
            <div className="rounded-xl border border-amber-500/50 bg-amber-500/5 p-5">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <h3 className="font-medium">
                    This account is connected to a different Orbit identity
                  </h3>
                  <p className="mt-2 text-sm text-muted-foreground">
                    Your Builderlab account owns communities under another Orbit
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/70 p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Orbit
                identity connected
                {identity.npub ? (
...
function UnpairIdentityButton({
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Unpair this Orbit identity?</AlertDialogTitle>
          <AlertDialogDescription>
            Your Builderlab account will no longer be connected to this Orbit
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            member. The recipient needs a connected Orbit identity first, and
