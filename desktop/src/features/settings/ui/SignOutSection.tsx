...
export function SignOutSection() {
      ...
      if (!fetchCancelledRef.current)
        setNsecError(
          err instanceof Error
            ? err.message
            : t("settings.signout.nsec-load-failed"),
        );
    } finally {
    ...
    signOut()
      ...
      .catch((err: unknown) => {
        ...
        toast.error(
          err instanceof Error
            ? err.message
            : t("settings.signout.submit-failed"),
        );
      });
  }
  ...
  return (
    <div className="mt-12 pb-6" data-testid="settings-signout">
      <SettingsOptionGroup title={t("settings.signout.title")}>
        <SettingsOptionRow>
          ...
          >
            {isPending ? (
              <Spinner
                aria-label={t("settings.signout.spinner-aria")}
                className="h-4 w-4 border-2"
              />
            ) : null}
            {isPending
              ? t("settings.signout.pending")
              : t("settings.signout.delete-button")}
          </Button>
        </SettingsOptionRow>
      </SettingsOptionGroup>
      <AlertDialog
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("settings.signout.dialog-title")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t("settings.signout.dialog-description")}
            </AlertDialogDescription>
          </AlertDialogHeader>

          <div className="space-y-3">
            <p className="text-sm font-medium">
              {t("settings.signout.backup-step")}
            </p>
            {isNsecLoading ? (
            ...
            >
              ...
              />
              <span>{t("settings.signout.backup-confirm")}</span>
            </label>
          </div>
          ...
          <div className="space-y-2">
            ...
            >
              {t("settings.signout.type-prefix")}{" "}
              <span className="font-semibold">"{SIGNOUT_CONFIRM_PHRASE}"</span>{" "}
              {t("settings.signout.type-suffix")}
            </label>
            <Input
              autoComplete="off"
