...
export function useSendMessageMutation(
  ...
  >({
    mutationFn: async ({
      ...
      // Messages carrying media, custom-emoji, link-preview, OR supersedes
      // tags MUST go through REST so the relay's tag validation runs. The
      // WebSocket path emits no extra tags, so these would otherwise be
      // silently dropped.
      if (
        forceRest ||
        transport === "http" ||
        ...
        const result = await sendChannelMessage(
          ...
          linkPreviewTags,
          sentFromThreadTag,
          // Upstream's tenant-scope guard owns positions 11 and 12; this send
          // path does not capture either, so they are passed as undefined.
          undefined,
          undefined,
          // 0.5.20's rootEventId (position 13), resolved from cache above.
          suppliedRootEventId,
          supersedesTags,
