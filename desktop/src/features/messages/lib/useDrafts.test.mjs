...
// ── nav-switch unmount round-trip ─────────────────────────────────────────────
//
// Regression for the image-drop bug on top-level nav switch (Inbox / Agents).
//
// When the user navigates FROM a channel TO a top-level nav destination
// (and back), the full ChannelPane+MessageComposer subtree unmounts and
// remounts.  The lifecycle that must hold:
//
//  1. OUTGOING (Channel A → Inbox): MessageComposer's effect cleanup fires.
//     persistDraftEntry is called with the full draft including pendingImeta.
//
//  2. INCOMING (Inbox → Channel A): MessageComposer mounts fresh.  The effect
//     body calls media.setPendingImeta(saved.pendingImeta) — this is an ASYNC
//     state update.  React StrictMode (dev mode) immediately simulates unmount
//     by running the cleanup before the state update commits, so
//     media.pendingImetaRef.current is still [] at that point.
//
//     The fix: MessageComposer also sets pendingImetaForPersistRef.current
//     SYNCHRONOUSLY in the effect body, so the cleanup closure reads the
//     synchronously-set value ([image]) instead of the unconmmitted state ([]).
//
//     The store-layer contract this regression test verifies:
//     • If the fix works, the draft stored after a mount+restore+simulated-
//       unmount sequence still contains the original images.
//     • If someone removes the synchronous ref write from the effect body
//       (reverting the fix), the StrictMode simulate-unmount will persist
//       pendingImeta=[] and overwrite the images — this test catches that.
//
// Note: the "simulate-unmount overwrites with empty imeta" scenario (the bug)
// is tested below as a negative case, so a reversion is immediately visible.

test("nav_switch_outgoing_persist_preserves_images_in_store", () => {
  // Simulate step 1: outgoing cleanup from Channel A with images.
  setup("pubkey-nav");
  persistDraftEntry("chan-nav", "draft with image", "chan-nav", [IMG_A], []);

  const saved = loadDraftEntry("chan-nav");
  assert.ok(saved, "draft should exist after outgoing persist");
  assert.equal(saved.pendingImeta.length, 1, "image preserved in store");
  assert.equal(saved.pendingImeta[0].url, IMG_A.url, "correct image URL");
  assert.equal(saved.content, "draft with image");
});

test("nav_switch_incoming_restore_then_correct_persist_preserves_images", () => {
  // Simulate the FIXED flow:
  // 1. Draft saved on outgoing nav (Channel A → Inbox).
  // 2. On remount (Inbox → Channel A): effect body reads draft, sets local
  //    pendingImetaForPersistRef.current = [IMG_A] (synchronous), then queues
  //    setPendingImeta([IMG_A]) (async).
  // 3. The cleanup (which the fix makes read from pendingImetaForPersistRef)
  //    sees [IMG_A] and persists correctly — even before the state commits.
  //
  // At the store layer this is: persist-with-images → load → persist-with-same-images.
  setup("pubkey-nav-restore");
  persistDraftEntry("chan-nav2", "text", "chan-nav2", [IMG_A], []);

  const saved = loadDraftEntry("chan-nav2");
  assert.ok(saved);

  // Simulate what the FIXED cleanup does: re-persists using the synchronously
  // captured pendingImeta (saved.pendingImeta), not the uninitialized [].
  persistDraftEntry(
    "chan-nav2",
    saved.content,
    saved.channelId,
    saved.pendingImeta, // synchronous ref — has the image
    saved.spoileredAttachmentUrls,
  );

  const after = loadDraftEntry("chan-nav2");
  assert.ok(after, "draft must still exist after re-persist");
  assert.equal(
    after.pendingImeta.length,
    1,
    "image survived the mount+cleanup cycle",
  );
  assert.equal(after.pendingImeta[0].url, IMG_A.url);
});

test("nav_switch_incoming_strictmode_bug_overwrites_images_when_cleanup_reads_stale_ref", () => {
  // Documents the PRE-FIX (buggy) scenario:
  // 1. Draft saved correctly with image on outgoing nav.
  // 2. On remount, StrictMode cleanup fires with pendingImetaRef.current = []
  //    (async state update not yet committed) and overwrites the draft.
  //
  // This test SHOULD remain passing — it documents the bug behavior.
  // If the overall fix ever needs to be revisited, failing this test means
  // the "empty overwrite" path would no longer corrupt the draft — but only
  // because something else changed, which needs careful review.
  setup("pubkey-nav-bug");
  persistDraftEntry("chan-nav3", "text", "chan-nav3", [IMG_A], []);

  // The bug: StrictMode cleanup persists with pendingImeta=[] (stale ref value).
  persistDraftEntry("chan-nav3", "text", "chan-nav3", [], []);

  const after = loadDraftEntry("chan-nav3");
  assert.ok(after, "draft exists even after overwrite (content non-empty)");
  assert.equal(
    after.pendingImeta.length,
    0,
    "BUG DOCUMENTED: empty-imeta persist overwrites saved images — fix prevents this from occurring",
  );
});
