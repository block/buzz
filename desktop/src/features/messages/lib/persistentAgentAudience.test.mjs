...
function createStorage(onSetItem = () => {}, initialValues = []) {
...
const enabledStorageKey = "buzz:keep-addressed-agents-active:v2";
const audienceStorageKey = "buzz:persistent-agent-audiences:v3";
const legacyEnabledStorageKey = "buzz:keep-addressed-agents-active";
const legacyAudienceStorageKey = "buzz:persistent-agent-audiences:v2";

let loadSequence = 0;

async function loadStore(offset = 0, initialValues = []) {
  globalThis.window = { localStorage: createStorage(() => {}, initialValues) };
  loadSequence += 1;
  return import(
...
function savedAudiences() {
  return JSON.parse(window.localStorage.getItem(audienceStorageKey));
}
...
test("an unchanged touch refreshes LRU without revision or emit", async () => {
  ...
  assert.equal(writes[0][0], audienceStorageKey);
...
test("legacy persistence starts disabled with an empty audience", async () => {
  const legacyScope = `${ownerA}:channel-a:thread:legacy`;
  const store = await loadStore(12, [
    [legacyEnabledStorageKey, "1"],
    [legacyAudienceStorageKey, JSON.stringify({ [legacyScope]: [agentA] })],
  ]);

  store.promotePersistentAgentAudience({
    expectedGeneration: store.getPersistentAgentAudienceGeneration(),
    expectedRevision: store.getPersistentAgentAudienceRevision(legacyScope),
    explicitAgentPubkeys: [agentA],
    scope: legacyScope,
  });
  assert.equal(window.localStorage.getItem(enabledStorageKey), null);
  assert.equal(window.localStorage.getItem(audienceStorageKey), null);

  store.setPersistentAgentAudienceEnabled(true);
  store.setPersistentAgentAudience(legacyScope, [agentB]);
  assert.equal(window.localStorage.getItem(enabledStorageKey), "1");
  assert.deepEqual(savedAudiences(), { [legacyScope]: [agentB] });
});
