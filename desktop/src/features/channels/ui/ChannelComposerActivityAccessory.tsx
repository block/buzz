...
/**
 * Leaf activity chrome for the main composer dock. Subscribes to working-agent
 * and card-mint stores here (not in ChannelPane) so typing/working churn cannot
 * re-render the timeline or MessageComposer.
 */
export const ChannelComposerActivityAccessory = React.memo(
  function ChannelComposerActivityAccessory({
    agents,
    channel,
    currentPubkey,
    onOpenAgentSession,
    ...
    const { hasActivity, workingBotPubkeys } = useComposerDockActivity(
      channel?.id ?? null,
      typingPubkeys,
    );
