...
for (const [owner, role] of [
  ...
  test(`fresh create/add then next @ catches up without a manual directory refresh (${owner === VIEWER ? "owned" : "nonowned"}, ${role})`, async () => {
    ...
    const installedKeys = mention.suggestions.map((row) => row.pubkey);
    ...
    assert.deepEqual(
      ...
      installedKeys,
      "membership refresh must not expand the installed chooser",
    );
