...
export function AppShell() {
  ...
  React.useEffect(() => {
    ...
    if (!channelIsAvailable) {
      ...
      void navigation.goHome({ replace: true });
      return;
    }
    ...
    if (selectedView === "home") {
      void navigation.goChannel(destination.channelId, { replace: true });
    }
  }, [
    channelsQuery.dataUpdatedAt,
    channelsQuery.isSuccess,
    communitiesHook.activeCommunity?.id,
    navigation.goChannel,
    navigation.goHome,
    selectedView,
    sidebarChannels,
  ]);
  ...
  const {
    ...
    openSearchHit: navigation.openSearchHit,
    pubkey: identityQuery.data?.pubkey,
  ...
  const handleCreateChannel = React.useCallback(
    async (
      ...
      await navigation.goChannel(createdChannel.id);
      onCreated?.(createdChannel.id);
      void applyAgents(templateId, createdChannel.id);
    },
    [applyAgents, applyCanvas, createChannelMutation, navigation.goChannel],
  );
  const handleCreateForum = React.useCallback(
    async ({
      ...
      await navigation.goChannel(createdForum.id);
      void applyAgents(templateId, createdForum.id);
    },
    [applyAgents, applyCanvas, createForumMutation, navigation.goChannel],
  );
  ...
  const handleHideDm = React.useCallback(
    ...
    [navigation.goHome, hideDmMutation, selectedChannelId],
  );
  const handleOpenSettings = React.useCallback(
    (section: SettingsSection = DEFAULT_SETTINGS_SECTION) => {
      setIsChannelManagementOpen(false);
      void navigation.goSettings(section);
    },
  ...
  const handleSettingsSectionChange = React.useCallback(
    (section: SettingsSection) => {
      void navigation.goSettings(section, { replace: true });
    },
    [navigation.goSettings],
  );
  ...
  const handleOpenSearchResult = React.useCallback(
    (hit: SearchHit) => {
      void navigation.openSearchHit(hit);
    },
    [navigation.openSearchHit],
  );
              ...
              <AppProfilePanelProvider>
                  ...
                  <div className="relative flex min-h-0 flex-1 overflow-visible">
                    {!isHuddleRoom ? (
                      <AppSidebar
                        ...
                        onNewMessage={navigation.goNewMessage}
                        ...
                        onOpenDm={async ({ pubkeys }) => {
                          ...
                          await navigation.goChannel(directMessage.id);
                        }}
                        onSelectAgents={() => void navigation.goAgents()}
                        onSelectBattleRhythm={() =>
                          void navigation.goBattleRhythm()
                        }
                        onSelectChannel={handleSidebarChannelSelect}
                        onSelectCommandConsole={() =>
                          void navigation.goCommandConsole()
                        }
                        ...
                        onSelectWorkflows={() => void navigation.goWorkflows()}
                ...
                <AppShellOverlays
                  ...
                  onSelectChannel={(channelId) => {
                    void navigation.goChannel(channelId);
                  }}
                />
