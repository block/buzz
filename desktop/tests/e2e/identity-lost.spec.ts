...
test("normal first launch uses the already-persisted identity", async ({
  ...
  await expect(gate).toHaveCSS("background-color", "rgb(216, 180, 254)");
  ...
  await expect(gate).toHaveCSS(
    "background-image",
    /radial-gradient\(.*\), linear-gradient\(.*rgb\(216, 180, 254\).*rgb\(215, 231, 246\)\)/s,
  );
  await expect(gate).toHaveCSS("color", "rgb(23, 23, 23)");
...
test("lost boot offers phone recovery with a single-use QR", async ({
  ...
  await expect(
    page.getByText("Scan this code with a signed-in Mesh phone."),
  ).toBeVisible();
  await expect(
...
test("phone recovery uses the desktop pairing card semantics", async ({
  ...
  await expect(
    card.getByText(
      "This gives this desktop permanent access to your Mesh identity. Only continue if you trust it.",
    ),
  ).toBeVisible();
  await expect(
