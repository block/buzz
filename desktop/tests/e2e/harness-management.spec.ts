...
const HERMES_AVAILABLE = {
  ...
  default_args: [],
  mcp_command: null,
  install_hint: "Zorro talks to Hermes Agent through its hermes-acp command.",
...
const OPENCLAW_NOT_INSTALLED = {
  ...
  mcp_command: null,
  install_hint:
    "Zorro talks to OpenClaw through its ACP mode (openclaw acp), which relies on the OpenClaw Gateway daemon. Follow the setup guide to install both.",
...
const CURSOR_AVAILABLE = {
  ...
  mcp_command: null,
  install_hint:
    "Zorro talks to Cursor through the cursor-agent CLI's ACP mode.",
