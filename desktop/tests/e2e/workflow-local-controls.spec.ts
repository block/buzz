...
async function addMessageStep(
  ...
  // Target the step's own textarea by id: a message-text trigger condition
  // also labels its input "Message text", and filling by label before the
  // inspector hands over from the trigger to the new step typed the step body
  // into the trigger condition, leaving the step textless and the workflow
  // unparseable as a form on reopen.
  await dialog
