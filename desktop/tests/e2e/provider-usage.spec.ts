...
test("provider allowance stays off until the user opts in", async ({
  ...
  await expect(page.getByTestId("provider-allowance-disabled")).toContainText(
    "Provider reads are off",
  );
...
test("owner dashboard combines account allowance with per-agent runtime metadata", async ({
  page,
}) => {
  const codexPubkey = "c".repeat(64);
  const goosePubkey = "d".repeat(64);
  await installMockBridge(page, {
    managedAgents: [
      {
        pubkey: codexPubkey,
  ...
  const section = page.getByTestId("agent-provider-allowance-section");
  await expect(section).toBeVisible();
  await expect(section).toContainText("Owner-private subscription limits");
  await expect(section.getByTestId("provider-allowance-summary")).toContainText(
    "48% remaining",
  );
  await expect(section.getByTestId("provider-allowance-summary")).toContainText(
    "Weekly · Resets",
  ...
  await expect(section.getByTestId("provider-allowance-summary")).toContainText(
    "shared by 1 matching agent",
  );

  const codexRow = section.getByTestId(
    `provider-allowance-agent-${codexPubkey}`,
  );
  await expect(codexRow).toContainText("Review Bot");
  await expect(codexRow).toContainText("Codex · openai · gpt-5.6");
  await expect(codexRow).toContainText("48% remaining");
  await expect(codexRow).toContainText("Shared account allowance");

  const unsupportedRow = section.getByTestId(
    `provider-allowance-agent-${goosePubkey}`,
  );
  await expect(unsupportedRow).toContainText("Goose · ollama · qwen3");
  await expect(unsupportedRow).toContainText("Unavailable from provider");

  await section.screenshot({
    path: "test-results/provider-usage-screenshots/01-owner-dashboard.png",
  });
});
...
test("provider allowance stays compact, theme-native, and accessible", async ({
  ...
  await expect
    ...
    .toEqual({ rootFontSize: "16px", textRemSize: "24px" });
