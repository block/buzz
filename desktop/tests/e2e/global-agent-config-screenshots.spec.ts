...
test.describe("global agent config screenshots", () => {
  ...
  test("04-create-blocked-no-provider-no-global", async ({ page }) => {
    ...
    await expect(harness).toHaveText("Zorro Agent");
  ...
  test("unset defaults persist the visible Zorro Agent fallback", async ({
    page,
  }) => {
    await installMockBridge(page);
    await openCreateDialog(page);
  ...
  test("create defaults follow a preferred harness saved while open", async ({
    ...
    const defaults = page.getByTestId("agent-ai-defaults-notice");
    await expect(defaults).toContainText("Zorro Agent");
    await defaults
  ...
  test("create exposes setup guidance when no harness is available", async ({
    ...
    await selectDropdownOption(page, harness, "Zorro Agent (not installed)");
    await expect(
      customSection
        .locator("p")
        .filter({ hasText: "Zorro Agent is not installed." }),
    ).toContainText(
      "Zorro Agent is not installed. Ships with the Zorro desktop app. Visit Settings > Agents to set it up.",
    );
    await expect(page.getByTestId("persona-dialog-submit")).toBeDisabled();
  });
  ...
  test("05-create-enabled-with-global-provider", async ({ page }) => {
    ...
    await expect(
      defaultsSection.getByText("Zorro Agent", { exact: true }),
    ).toBeVisible();
