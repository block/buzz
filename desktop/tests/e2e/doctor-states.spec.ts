...
const CODEX_NOT_INSTALLED = {
  ...
  default_args: [],
  mcp_command: null,
  install_hint: "Mesh talks to Codex through the Codex CLI.",
...
test.describe("Doctor panel state screenshots", () => {
  ...
  test("00-runtime-card-layout", async ({ page }) => {
    ...
    await expect(page.getByTestId("doctor-runtime-codex")).not.toContainText(
      "Mesh talks to Codex through the Codex CLI.",
    );
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              stdout: "",
              stderr:
                "The installer finished, but Mesh still could not use codex (observed: NotInstalled).",
              exit_code: null,
              hint: "Mesh requires the vendor CLI executable, not only its desktop app. If the CLI was installed while Mesh was open, restart Mesh and check again.",
            },
          ],
        },
      ],
