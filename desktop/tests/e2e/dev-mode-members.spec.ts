...
// Channels can have 1000+ members: the top bar must collapse to a count
// instead of listing names, and the palette's member browser must stay
// responsive by capping rendered rows until the query narrows the roster.
test("dev mode collapses large member lists to a searchable count", async ({
  page,
}) => {
  await installMockBridge(page);
  await page.addInitScript(() => {
  ...
  // 4 seeded members render as names.
  const members = page.getByTestId("dev-mode-channel-members");
  await expect(members).toContainText("alice");
  ...
  // Grow the roster past the inline threshold, then emit a membership
  // event — the same live path a real join takes — to refresh the queries.
  await page.evaluate(
    ({ channel, actor }) => {
      window.__BUZZ_E2E_ADD_MOCK_MEMBERS__?.({
        channelName: channel,
        count: 200,
      });
      window.__BUZZ_E2E_EMIT_MOCK_MESSAGE__?.({
  ...
  await expect(members).toContainText("204 members");

  // Clicking the count opens the palette's member browser: rendered rows
  // are capped with an overflow hint, and typing narrows the roster.
  await members.click();
  const paletteInput = page.getByTestId("dev-mode-palette-input");
  await expect(paletteInput).toHaveAttribute(
    "placeholder",
    /search members of # general/,
  );
  const entries = page.getByTestId("dev-mode-palette-entry");
  await expect(entries).toHaveCount(41);
  await expect(entries.last()).toContainText("more — type to narrow");

  await paletteInput.fill("alice");
  await expect(entries).toHaveCount(1);
  await expect(entries.first()).toContainText("alice");
});
