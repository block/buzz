...
test("fresh existing-identity path leads with private-key recovery", async ({
  ...
  await expect(
    page.getByText("Paste your private key to sign in to Zorro."),
  ).toBeVisible();
  ...
  await expect(
    phoneDialog.getByRole("heading", { name: "Use your Zorro identity" }),
  ).toBeVisible();
...
test("first-community owner can create and connect a hosted community", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "Finish connecting Zorro" }),
  ).toBeVisible();
...
test("first-community owner can replace a mismatched account identity", async ({
  ...
  await expect(
    page.getByRole("heading", {
      name: "This account uses a different Zorro identity",
    }),
  ).toBeVisible();
  await page
...
test("denied on relay A then paste relay B invite URL switches community to B", async ({
  ...
  await page
    .getByLabel("I agree to the Zorro Terms of Service and Privacy Policy.")
    .check();
  await page.getByTestId("invite-redeem-submit").click();
