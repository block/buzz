...
const overrides = new Map([
  ...
  ["src/features/channels/useUnreadChannels.ts", 865], // NIP-RS read marker tracking + participated/authored/followed thread ID sets + localStorage persistence + catch-up REQ with thread activity collection + thread reply activity feed items + mutedRootIds denylist with localStorage persistence + muteThread/unmuteThread callbacks + markChannelRead latestByChannelRef fallback chain (matches markChannelUnread) + two-tier badge: latestHighPriorityByChannelRef + highPriorityUnreadChannelIds memo tracking DMs + @mentions + broadcast replies + markChannelRead observed-latest ref clearing + Set identity stabilization for downstream memo perf
