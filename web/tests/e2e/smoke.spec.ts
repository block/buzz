...
test("home page loads with Zorro branding", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("main").getByRole("img", { name: "Zorro" }),
  ).toBeVisible();
});
...
test("invite requires age and legal consent before opening Zorro", async ({
  page,
}) => {
  ...
  const agreementConfirmation = page.getByLabel(
    "I agree to the Zorro Terms of Service and Privacy Policy.",
  );
  const acceptInvite = page.getByRole("button", {
    name: "Accept invite in Zorro",
  });
  ...
  await page
    .locator("label")
    .filter({
      hasText: "I agree to the Zorro Terms of Service and Privacy Policy.",
    })
