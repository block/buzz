...
export function validateCodex(value: CodexSetup): void {
  if (!value || !isAbsolute(value.cli) || !isAbsolute(value.apiKeyFile) || !Array.isArray(value.models) || !value.models.length || value.models.length > 100 || value.models.some(m => typeof m !== 'string' || !/^[a-zA-Z0-9_.:/-]{1,200}$/.test(m))) throw Error('Invalid local Codex CLI/key-file/models binding');
}
/** Build a closed service-user env. A local key is a prerequisite, NOT authentication proof. */
export function codexEnvironment(value: CodexSetup, home: string, configDirectory: string, model: string): Record<string, string> {
  validateCodex(value);
  try {
  ...
  } catch { throw Error('Codex API key prerequisite missing/unsafe locally; provision an owner-only key file as the service user. No login initiated'); }
