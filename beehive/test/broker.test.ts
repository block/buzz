...
test('broker enforces model, completion health, cancellation and owned teardown across escaped shims', async () => {
  for (const mode of ['ok', 'reject', 'wrong-model', 'conflict', 'bad-tail', 'delayed', 'cancel', 'foreign-workspace', 'injected-mcp']) {
    ...
    const session = new ConversationSession({ executable: runtime, relay: 'ws://127.0.0.1:1' }, {
