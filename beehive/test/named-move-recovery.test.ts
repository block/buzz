...
for (const failure of ['changed-across-target-restart', 'remote-candidate-save'] as const) test(`named profiled Move reserved revision; post-grant ${failure} leaves target assigned stopped`, { timeout: 20000 }, async () => {
  ...
  try {
    ...
    ui.send(message('save','target',agent,0,{ configurationAction: 'create', name: 'Destination' }));
    ...
    else { assert.deepEqual(journal('target').configurations.Destination, consumed.selection); assert.equal(journal('target').selected.behavior.instructions, behavior.instructions); }
