...
class FakeFlutterProcess:
    def __init__(self, returncode=0): self.returncode, self.running = returncode, True
    def poll(self): return None if self.running else self.returncode
    def wait(self, timeout=None): self.running = False; return self.returncode
...
class IosReviewTests(unittest.TestCase):
    ...
    def test_recording_readiness_fails_without_rendered_marker(self):
        ...
        with tempfile.TemporaryDirectory() as directory, \
            ...
            with self.assertRaisesRegex(ios_review.ReviewError, "timed out waiting"):
                ios_review.wait_for_recording_ready(process, pathlib.Path(directory) / "flutter.log", 1)

    def test_recording_readiness_accepts_rendered_marker(self):
        process = FakeFlutterProcess()
        with tempfile.TemporaryDirectory() as directory:
            log = pathlib.Path(directory) / "flutter.log"
            log.write_text(ios_review.RECORDING_READY_MARKER)
            ios_review.wait_for_recording_ready(process, log, 0.1)
